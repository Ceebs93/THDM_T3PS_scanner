                            ...
                            if (corr) then
                                ...
                                if (anticorrmu) then
                                    if (analyses(i)%table%collaboration .eq. analyses(j)%table%collaboration) then
                                        cov_mu_tot(iii, jjj) = cov_mu_tot(iii, jjj) + &
                                                               (analyses(i)%table%dlumi*analyses(j)%table%dlumi)* &
                                                               (analyses(i)%peaks(ii)%mu* &
                                                                analyses(j)%mpred%mp_Higgses(mpred_j)%mu)
                                    endif
                                    cov_mu_tot(iii, jjj) = cov_mu_tot(iii, jjj) + &
                                                           (dratesq)*(analyses(i)%peaks(ii)%total_mu* &
                                                                      analyses(j)%mpred%mp_Higgses(mpred_j)%total_mu)
                                else
                                    if (analyses(i)%table%collaboration .eq. analyses(j)%table%collaboration) then
                                        cov_mu_tot(iii, jjj) = cov_mu_tot(iii, jjj) + &
                                                               (analyses(i)%table%dlumi*analyses(j)%table%dlumi)* &
                                                               abs(analyses(i)%peaks(ii)%mu* &
                                                                   analyses(j)%mpred%mp_Higgses(mpred_j)%mu)
                                    endif
                                    cov_mu_tot(iii, jjj) = cov_mu_tot(iii, jjj) &
                                                           + (dratesq)*abs(analyses(i)%peaks(ii)%total_mu* &
                                                                           analyses(j)%mpred%mp_Higgses(mpred_j)%total_mu)
                                endif
                            endif
!-(3)------------------------------------------------------------------------------------
                        elseif (ii .gt. size(analyses(i)%peaks, dim=1) .and. &
                            ...
                            if (corr) then
                                call get_rate_uncertainties_sq_peak_mpred(dratesq, analyses(i)%table, &
                                                                          analyses(i)%mpred%mp_Higgses(mpred_i), &
                                                                          analyses(j)%peaks(jj))
                                if (anticorrmu) then
                                    if (analyses(i)%table%collaboration .eq. analyses(j)%table%collaboration) then
                                        cov_mu_tot(iii, jjj) = cov_mu_tot(iii, jjj) + &
                                    ...
                                    cov_mu_tot(iii, jjj) = cov_mu_tot(iii, jjj) &
                                                           + (dratesq)*(analyses(j)%peaks(jj)%total_mu &
                                                                        *analyses(i)%mpred%mp_Higgses(mpred_i)%total_mu)
                                else
                                    if (analyses(i)%table%collaboration .eq. analyses(j)%table%collaboration) then
                                        cov_mu_tot(iii, jjj) = cov_mu_tot(iii, jjj) + &
                                                               (analyses(i)%table%dlumi*analyses(j)%table%dlumi)* &
                                                               abs(analyses(j)%peaks(jj)%mu &
                                                                   *analyses(i)%mpred%mp_Higgses(mpred_i)%mu)
                                    endif
                                    cov_mu_tot(iii, jjj) = cov_mu_tot(iii, jjj) + &
                                                           (dratesq)*abs(analyses(j)%peaks(jj)%total_mu &
                                                                         *analyses(i)%mpred%mp_Higgses(mpred_i)%total_mu)
                                endif
                            endif
!-(4)------------------------------------------------------------------------------------
...
!-----Treat luminosity uncertainty as correlated systematic error if same collaboration:
                                    ...
                                    cov_mu_tot(iii, jjj) = cov_mu_tot(iii, jjj) + (dratesq)* &
                                                           abs(analyses(i)%mpred%mp_Higgses(mpred_i)%total_mu &
                                                               *analyses(j)%mpred%mp_Higgses(mpred_j)%total_mu)
                                endif
                            endif
!----Add the intrinsic (uncorrelated) uncertainty of this peak to the diagonal elements:
...
!------------------------------------------------------------------------------------
        ...
        do i = 1, table%Nc
            do j = 1, peak%Nc
                p1 = table%channel_p_id(i)
                d1 = table%channel_d_id(i)
                p2 = peak%channel_p_id(j)
                d2 = peak%channel_d_id(j)
!
