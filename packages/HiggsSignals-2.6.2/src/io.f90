...
!--------------------------------------------------------------------
        use usefulbits, only: np, ndat, n_additional, theo, effC, effC_Hc, g2, partR, &
                              ...
                              allocate_couplratio_parts, allocate_couplratio_Hc_parts, &
...
!--------------------------------------------------------------------
        ...
        use usefulbits, only: np, pdesc, whichinput
        use usefulbits_hs, only: Exptdir, pdf
        ...
        !Exptdir, pdf, whichinput, np(Hneut), np(Hplus) , prefix
        nargs_datfile = 3 + 2 + 1
        np(Chineut) = 0
        np(Chiplus) = 0
...
!--------------------------------------------------------------------
        ...
        write (*, *) ' ./HiggsSignals <expdata> <pdf> <whichinput> <nHzero> <nHplus> <prefix>'
        write (*, *) 'e.g.'
        write (*, *) ' ./HiggsSignals latestresults 2 effC 3 1 example_data/mhmodplus/mhmod+_'
...
!--------------------------------------------------------------------
        use usefulbits, only: theo, ndat, inputmethod, np, &
                              n_additional, file_id_common, file_id_common2, &
                              whichinput, listprocesses, infile1
        use usefulbits_hs, only: analyses, StrCompress, pdf, HSres, Exptdir, &
                                 Nparam
        ...
        case ('datfile')
            ...
            case ('effC', 'part', 'hadr')
                ...
                write (file_id_common, *) '# generated with HiggsSignals version '// &
                    trim(adjustl(version))//' on '//createdateandtime
                write (file_id_common, *) '# settings: '//adjustl(trim(Exptdir))//', '//whichinput//', ' &
                    //adjustl(trim(pdf_desc(pdf)))
                ...
                if (n_additional > 0) then
                    do jj = 1, ndat
                        write (file_id_common, fmt=trim(format43)) jj, (Mhall(jj, i), i=1, np(Hneut) + np(Hplus)), &
                            ...
                            HSres(jj)%nobs_mu, HSres(jj)%nobs_mh, HSres(jj)%nobs, HSres(jj)%Pvalue, &
...
!! @param obsID ID of the observable of interest
!! @param Nc number of channels contributing to the observable
!! @param pIDs array of [production IDs of the channels](doc/channels.md)
!! @param dIDs array of [decay IDs of the channels](doc/channels.md)
!! @param efficiencies array of the channel efficiencies
...
!--------------------------------------------------------------------
        use usefulbits, only: inputmethod, whichinput, infile1
...
!************************************************************
    subroutine HiggsSignals_outputSLHAdata(infile)
        ...
        use usefulbits_hs, only: HSres, pdf, Exptdir, SLHAdetailed, &
                                 newSLHAfile
        ...
        if (ios .ne. 0) then
            ...
            write (k_out, '(3X,I2,17X,A2,A,A2,15X,A)') 0, '||', trim(adjustl(version)), '||', &
