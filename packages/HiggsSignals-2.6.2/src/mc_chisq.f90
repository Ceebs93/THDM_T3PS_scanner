...
!----------------------------------------------------------------------
        ...
        integer :: i, j, imin, imax, step
        double precision :: mh, val(3), boxstart, boxend, chisq, minchisq, mu_selected(3), &
            mh_selected, dm, dmu
        ...
        do j = lbound(mp_H, dim=1), ubound(mp_H, dim=1)
            ...
            if (abs(boxstart - boxend) .lt. vsmall) then
...
!---gaussian theory uncertainty
                ...
                case DEFAULT
                    write(*,*) "invalid pdf in scan_mutable"
                    stop 1
                end select
...
!----------------------------------------------------------------------
        ...
        integer :: i, ii, j, jj, jjj, k, xmin, xmax, step, best_Hindex
        logical, allocatable :: testHiggs(:, :)
        logical, allocatable :: Higgstested(:)
        character(LEN=100) :: filename
        character(LEN=1) :: pdfstring

        double precision :: mh, val(3), dm, dmu, sep, minchisq2, lowestmass, highestmass
...
! Unphysical starting values.
        ...
        do j = lbound(analyses, dim=1), ubound(analyses, dim=1)
            do jj = lbound(analyses(j)%mpred%mp_Higgses, dim=1), &
                ...
                jjj = jjj + 1
                if (analyses(j)%table%xmin .le. xmin) xmin = int(analyses(j)%table%xmin)
                if (analyses(j)%table%xmax .ge. xmax) xmax = int(analyses(j)%table%xmax)
...
!-and add the signal strength modifiers. Untag the combined Higgs bosons.
            if (abs(mp_H(index_i)%dm) .lt. vsmall .and. abs(mp_H(index_j)%dm) .lt. vsmall) then
                dm_tmp = 0.0D0
            else
                dm_tmp = mp_H(index_i)%dm*mp_H(index_j)%dm/ &
                         sqrt(mp_H(index_i)%dm**2 + mp_H(index_j)%dm**2)
            endif
            if (abs(dm_tmp) .gt. 0.0D0) then
                m_tmp = dm_tmp**2*(mp_H(index_i)%m/mp_H(index_i)%dm**2 + &
                                   mp_H(index_j)%m/mp_H(index_j)%dm**2)
            else
                if (abs(mp_H(index_i)%dm) .lt. vsmall .and. abs(mp_H(index_j)%dm) .ge. vsmall) then
                    m_tmp = mp_H(index_i)%m
                else if (abs(mp_H(index_i)%dm) .ge. vsmall .and. abs(mp_H(index_j)%dm) .lt. vsmall) then
                    ...
                    do jj = lbound(analyses(j)%mpred%mp_Higgses, dim=1), &
                        ...
                        if (correlations_mu .or. (.not. correlations_mu .and. iii .eq. jjj)) then
                            call get_rate_uncertainties_sq_mpred(dratesq, analyses(i)%table, &
                                                                 analyses(i)%mpred%mp_Higgses(ii), &
                                                                 analyses(j)%table, analyses(j)%mpred%mp_Higgses(jj))
...
! (TS 28/10/2013: Change scaling of theo. err from observed to predicted mu:)
                                cov_mp(iii, jjj) = cov_mp(iii, jjj) + (dratesq)* &
                                                   (analyses(i)%mpred%mp_Higgses(ii)%total_mu &
                                                    *analyses(j)%mpred%mp_Higgses(jj)%total_mu)
                            else
                                cov_mp(iii, jjj) = cov_mp(iii, jjj) + (dratesq)* &
                                                   abs(analyses(i)%mpred%mp_Higgses(ii)%total_mu &
                                                       *analyses(j)%mpred%mp_Higgses(jj)%total_mu)
                            endif
                        endif
