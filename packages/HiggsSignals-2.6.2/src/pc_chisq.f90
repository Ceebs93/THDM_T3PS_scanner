...
!! integer, allocatable :: indices_best(:)
        double precision :: force_in_range, forced_mass_range
            ...
            do i = lbound(peaks, dim=1), ubound(peaks, dim=1)
                do j = lbound(Higgs_to_be_assigned, dim=1), ubound(Higgs_to_be_assigned, dim=1)
                    if ((pdf .eq. 1) .or. (pdf .eq. 3)) then
                        forced_mass_range = peaks(i)%Higgses(j)%dm + force_in_range*table%deltam
                    else
                        forced_mass_range = force_in_range*sqrt(peaks(i)%Higgses(j)%dm**2 + table%deltam**2)
                    endif
                    ! write(*,*) "forced_mass_range = ",forced_mass_range
                    if (abs(peaks(i)%Higgses(j)%m - peaks(i)%mpeak) .le. forced_mass_range) then
...
!---Calculate the chi squared value for this combination assigned to peak i:
                    ...
                    ! write(*,*) "pccsq calc: ", pccsq, csq_mass_separation_weighted(peakindices(a, i, :), peaks(i))
...
! write(*,*) 'pccsq_tmp, chisq_fp = ',pccsq_tmp, chisq_fp
                        ...
                        if (Higgs_to_be_assigned(j) .eq. 1) then
                            do ii = lbound(peakindices, dim=2), ubound(peakindices, dim=2)
                                do jj = lbound(peakindices, dim=3), ubound(peakindices, dim=3)
                                    ! write(*,*) 'j = ', j,', peakindices(',a,',',ii,',',jj,') = ',peakindices(a, ii, jj)
...
!------------------------------------------------------------------------------------
        use usefulbits, only: div, np, Hneut, small
        implicit none
        type(mupeak) :: peak
        type(mutable) :: table
        integer :: j, k
...
!----Loop over the channels and add rates
                do k = lbound(peak%channel_mu, dim=1), ubound(peak%channel_mu, dim=1)
                    ! write(*,*) "evaluate_peak (1st loop): ", peak%id, k, peak%channel_mu(k),&
...
!--Reweight the rates to obtain the channel_mu
            ...
            ! write(*,*) "evaluate_peak: ", peak%id, k, peak%channel_mu(k), peak%channel_w_corrected_eff(k)
        enddo
...
!------------------------------------------------------------------------------------
    subroutine calculate_total_pc_chisq(csq_tot, csq_mu, csq_mh, N, Nmu, Nmh, signalScaling)
...
!------------------------------------------------------------------------------------
        implicit none
        double precision, allocatable :: csq_mu_in(:), csq_mh_in(:, :), csq0(:)
        ...
        integer :: i, ii, iii, j, jj, Niterations
        logical :: iterate
...
! call calculate_mu_chisq(csq0_nocorr, Nmu, 2)      ! csq0 without correlations
        ...
        ! perform signal scaling
        if (present(signalScaling)) then
            do i = lbound(analyses, dim=1), ubound(analyses, dim=1)
                do ii = lbound(analyses(i)%peaks, dim=1), ubound(analyses(i)%peaks, dim=1)
                    analyses(i)%peaks(ii)%total_mu = (1 + signalScaling*analyses(i)%peaks(ii)%total_mu - signalScaling)
                enddo
            enddo
        endif
...
!&         analyses(i)%peaks(ii)%domH.eq.0) then
                elseif (analyses(i)%peaks(ii)%domH .eq. 0 .and. .not. present(signalScaling)) then
                        ...
                        do k = 1, nH
                            ...
                            if (Hindex .ne. 0 .and. analyses(i)%table%mhchisq .eq. 1) then
                                csq_mh_out(Hindex, iii) = csq_mh(analyses(i)%peaks(ii)%Higgses(Hindex)%m, &
                                                                 ...
                                                                 analyses(i)%peaks(ii)%Higgses(Hindex)%dm, &
                                                                 analyses(i)%peaks(ii)%dm)
                            endif
                        enddo
                    endif
...
!------------------------------------------------------------------------------------
        ...
        integer :: Npeaks, i, ii, iii, j, jj, jjj, k
...
!    if((Hindex_i.eq.Hindex_j).and.(Hindex_i.ne.0).and.domax.eq.0) then
                            if (int_in_array(k, analyses(i)%peaks(ii)%Higgs_comb) &
                                .and. int_in_array(k, analyses(j)%peaks(jj)%Higgs_comb) .and. domax .eq. 0) then
...
!------------------------------------------------------------------------------------
        use usefulbits_hs, only: usescalefactor, symmetricerrors, &
...
! double precision, intent(in) :: dlumi    ! Uncertainty of Luminosity (relative)
        integer :: j
        double precision :: dcsq, dmulow0sq, dmuup0sq, mu
                    ...
                    do jj = lbound(analyses(j)%peaks, dim=1), ubound(analyses(j)%peaks, dim=1)
                        ...
                        if (corr .or. (.not. corr .and. iii .eq. jjj)) then
                            call get_rate_uncertainties_sq_peaks(dratesq, analyses(i)%peaks(ii), &
                                                                 ...
                                                                 analyses(i)%table%energy, analyses(j)%table%collider, &
                                                                 analyses(j)%table%energy)
...
!--------------------------------------------------------------------
    subroutine get_peakchisq(obsID, csqmu, csqmh, csqmax, csqtot)
!--------------------------------------------------------------------
        implicit none
    ...
    end subroutine get_peakchisq
!------------------------------------------------------------------------------------
    subroutine get_masschisq_from_separation(csq)
!------------------------------------------------------------------------------------
        implicit none
...
! write(*,*) 'csq_sep =', csq
    ...
    end subroutine get_masschisq_from_separation
!------------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------------
        use usefulbits_HS, only: mupeak, symmetricerrors
        implicit none
...
!--Internal parameters:
        double precision :: mutot, csq0, csq_mh_tot, csq_tot, csq_mh_tmp, mumax
...
!  write(*,*) "Warning!! called csq_mh_with_corr!"
            ...
            if (useaveragemass) then
                ! write(*,*) "before average_mass_for_peak : ", indices_in, peak%Higgses(indices_in)%mu
                call get_average_mass_for_peak(indices_in, peak, m, dm)
                ! write(*,*) "average_mass_for_peak : ", indices_in, m, dm
...
!------------------------------------------------------------------------------------
        ...
        if (pdf .eq. 1) then  ! box pdf
            dx = dx1 + max(assignmentrange, assignmentrange_massobs)*dx2  ! Add exp. and th. uncertainties linearly
...
!    write(*,*) "i, csq_mh_tmp = ", i , csq_mh_tmp
                    ...
                    if (pdf .eq. 1) then
                        if (peak%Higgses(i)%mu .ge. significantcontribution*mutotal) then
                            csq = csq + csq_mh_tmp
                        else
                            continue
                        endif
...
! use theory_XS_SM_functions
        ...
        double precision :: SMCS_lhc7_gg_H, SMCS_lhc7_bb_H, SMCS_lhc7_vbf_H, SMCS_lhc7_HW, &
            SMCS_lhc7_HZ, SMCS_lhc7_ttH, SMCS_lhc7_tH_tchan, SMCS_lhc7_tH_schan, SMCS_lhc7_qq_HZ, &
            SMCS_lhc7_gg_HZ, SMCS_lhc8_gg_H, SMCS_lhc8_bb_H, SMCS_lhc8_vbf_H, &
            SMCS_lhc8_HW, SMCS_lhc8_HZ, SMCS_lhc8_ttH, SMCS_lhc8_tH_tchan, SMCS_lhc8_tH_schan, &
            SMCS_lhc8_qq_HZ, SMCS_lhc8_gg_HZ, SMCS_lhc13_gg_H, SMCS_lhc13_bb_H, &
            SMCS_lhc13_vbf_H, SMCS_lhc13_HW, SMCS_lhc13_HZ, SMCS_lhc13_ttH, SMCS_lhc13_tH_tchan, &
            SMCS_lhc13_tH_schan, SMCS_lhc13_qq_HZ, SMCS_lhc13_gg_HZ, SMCS_lhc13_tWH, SMCS_tev_gg_H, &
            ...
            if (mutab%collider .eq. 'LHC') then
                if (abs(mutab%energy - 7.0D0) .le. small) then
                    if (p .eq. 1) then
                        ...
                        SMrate = SMCS_lhc7_gg_H(mass)
                    else if (p .eq. 7) then
                        SMrate = SMCS_lhc7_bb_H(mass)
                    else if (p .eq. 8) then
                        SMrate = SMCS_lhc7_tH_tchan(mass)
                    else if (p .eq. 9) then
                        SMrate = SMCS_lhc7_tH_schan(mass)
                    else if (p .eq. 10) then
                        SMrate = SMCS_lhc7_qq_HZ(mass)
                    else if (p .eq. 11) then
                        SMrate = SMCS_lhc7_gg_HZ(mass)
                    ...
                    if (p .eq. 1) then
                        ...
                        SMrate = SMCS_lhc8_gg_H(mass)
                    else if (p .eq. 7) then
                        SMrate = SMCS_lhc8_bb_H(mass)
                    else if (p .eq. 8) then
                        SMrate = SMCS_lhc8_tH_tchan(mass)
                    else if (p .eq. 9) then
                        SMrate = SMCS_lhc8_tH_schan(mass)
                    else if (p .eq. 10) then
                        SMrate = SMCS_lhc8_qq_HZ(mass)
                    else if (p .eq. 11) then
                        SMrate = SMCS_lhc8_gg_HZ(mass)
                    else if (p .eq. 12) then
                        SMrate = 0.0D0 ! n/a
                    else if (p .eq. 0) then
                        SMrate = 0.0D0
                    ...
                    if (p .eq. 1) then
                        ...
                        SMrate = SMCS_lhc13_gg_H(mass)
                    else if (p .eq. 7) then
                        SMrate = SMCS_lhc13_bb_H(mass)
                    else if (p .eq. 8) then
                        SMrate = SMCS_lhc13_tH_tchan(mass)
                    else if (p .eq. 9) then
                        SMrate = SMCS_lhc13_tH_schan(mass)
                    else if (p .eq. 10) then
                        SMrate = SMCS_lhc13_qq_HZ(mass)
                    else if (p .eq. 11) then
                        SMrate = SMCS_lhc13_gg_HZ(mass)
                    else if (p .eq. 12) then
                        SMrate = SMCS_lhc13_tWH(mass) ! Implemented function is not really mass-dependent
                    else if (p .eq. 0) then
                        SMrate = 0.0D0
                    endif
                endif
...
!---------------------------------------------------------------------------
        ...
        do k = 1, nH
            ...
            ! write(*,*) "Hindex = ", Hindex
            if (Hindex .ne. 0) then
                ! write(*,*) "peak%Higgses(Hindex)%mu = ", peak%Higgses(Hindex)%mu
                ...
                ! write(*,*) "average mass calc:", k, num1, num2, denom
            endif
        enddo
