...
module datatables
!--------------------------------------------------------------------
    use usefulbits_HS, only: mutable, mupeak, Exptdir, obs, analyses, corrlist !, withcorrexpsyst
!x  mutables, peaks
...
!--------------------------------------------------------------------
        use install_data_HS
        use usefulbits, only: np, file_id_common3, file_id_common2
!   implicit none
        character(LEN=100) :: datafile(500)
        integer n_datafiles, n_correlations, n_correlations_tmp

        !--------------------------------------input
!  type(mutable),allocatable :: tables(:)
        !-----------------------------------internal
        integer :: x, n, i, m, int1, int2
...
!   close(file_id_common3)
        call system('ls -1 -p '//trim(adjustl(pathname_HS))//'Expt_tables/'// &
                    trim(adjustl(Exptdir))//'/*.txt 2>/dev/null | xargs -L 1 basename > HS_analyses.txt 2>/dev/null')
...
!  endif
        ...
        print *, "Reading in measurements from the following datafiles from analysis-set "// &
...
!   read(file_id_common3,*) obs(n)%table%deltam
            ...
            allocate (obs(n)%table%channel_id_str(obs(n)%table%Nc))
!    allocate(obs(n)%table%channel_id(obs(n)%table%Nc))
            allocate (obs(n)%table%channel_p_id(obs(n)%table%Nc))
            allocate (obs(n)%table%channel_d_id(obs(n)%table%Nc))
!    read(file_id_common3,*) (obs(n)%table%channel_id(i),i=1,obs(n)%table%Nc)
            read (file_id_common3, *) (obs(n)%table%channel_id_str(i), i=1, obs(n)%table%Nc)
            do i = 1, obs(n)%table%Nc
                posperiod = index(obs(n)%table%channel_id_str(i), '.')
                if (posperiod .eq. 0) then
                    if (len(trim(adjustl(obs(n)%table%channel_id_str(i)))) .eq. 2) then
                        read (obs(n)%table%channel_id_str(i), *) id
                        obs(n)%table%channel_p_id(i) = int((id - modulo(id, 10))/dble(10))
                        obs(n)%table%channel_d_id(i) = modulo(id, 10)
                    else
                        write (*, *) " For observable ID = ", obs(n)%id
                        stop " Error: Cannot handle channel IDs!"
                    endif
                else
                    read (obs(n)%table%channel_id_str(i) (:posperiod - 1), *) obs(n)%table%channel_p_id(i)
                    read (obs(n)%table%channel_id_str(i) (posperiod + 1:), *) obs(n)%table%channel_d_id(i)
                endif
            enddo
!   write(*,*) "Production channels = ",obs(n)%table%channel_p_id
!   write(*,*) "Decay channels = ",obs(n)%table%channel_d_id
!    write(*,*) obs(n)%table%channel_id_str
        ...
        do x = 1, n_datafiles
            fullfilename = trim(adjustl(pathname_HS))//'Expt_tables/'//trim(adjustl(Exptdir))//'/' &
                           //trim(datafile(x))
            call read_mutable(obs(x)%table, skip(x), fullfilename)
        enddo
        deallocate (skip)

!NEW:
        call system('ls -1 -p '//trim(adjustl(pathname_HS))//'Expt_tables/'//trim(adjustl(Exptdir))// &
                    '/*.corr 2>/dev/null | xargs -L 1 basename > HS_correlations.txt 2>/dev/null')

        open (file_id_common3, file="HS_correlations.txt", form='formatted')
        ...
        do
            ...
            fullfilename = trim(adjustl(pathname_HS))//'Expt_tables/'//trim(adjustl(Exptdir))//'/' &
                           //trim(datafile(n))
            call system('cat '//trim(adjustl(fullfilename))//' | wc -l > HS_ncorrelations.txt')
            open (file_id_common2, file="HS_ncorrelations.txt", form='formatted')
        ...
        allocate (corrlist(n_correlations))
...
!     write(*,*) m, int1, int2, db1, ios
                if (ios .ne. 0) exit
                corrlist(m)%obsID1 = int1
                corrlist(m)%obsID2 = int2
                corrlist(m)%corr = db1
            enddo
            m = m - 1
            close (file_id_common3)
...
!--------------------------------------------------------------------
        ...
        integer :: indxstr, prevstr, beginningstr
...
!     obs(i)%peak%channel_id(:) = obs(i)%table%channel_id(:)
                if (.not. allocated(obs(i)%peak%channel_p_id)) then
                    allocate (obs(i)%peak%channel_p_id(obs(i)%peak%Nc))
                endif
                obs(i)%peak%channel_p_id(:) = obs(i)%table%channel_p_id(:)
                if (.not. allocated(obs(i)%peak%channel_d_id)) then
                    allocate (obs(i)%peak%channel_d_id(obs(i)%peak%Nc))
                endif
                obs(i)%peak%channel_d_id(:) = obs(i)%table%channel_d_id(:)
...
!--------------------------------------------------------------------
    subroutine read_mutable(t1, skip, fullfilename)
!--------------------------------------------------------------------
!--------------------------------------input
        type(mutable) :: t1
        integer :: skip
...
!--------------------------------------------------------------------
        ...
        if (allocated(obs)) then
            do x = lbound(obs, dim=1), ubound(obs, dim=1)
                ...
                deallocate (obs(x)%table%channel_id_str)
!    deallocate(obs(x)%table%channel_id)
                deallocate (obs(x)%table%channel_p_id)
                deallocate (obs(x)%table%channel_d_id)
...
!     deallocate(obs(x)%peak%channel_id)
                    deallocate (obs(x)%peak%channel_p_id)
                    deallocate (obs(x)%peak%channel_d_id)
