...
! 
! It includes routines for the running of alpha_s.
! 
c..
c..   runalpha.f
c..
c..   routines for running of \alpha_s and mq.
c..
...
C-{{{ subroutine polemass:
      ...
      subroutine polemass(mqmq,apimq,rnf,nloopin,mqpole)
c..
...
c..
      ...
      real*8 lmum,rnf,nf,mqpole,mqmq,apimq
...
c$$$      if (nloop.ge.3) then
c$$$         write(6,*) '<function polemass>: nloop=',nloop
c$$$     &        ,'not implemented. Using nloop=2.'
c$$$         nloop=2
c$$$      endif

      mqpole=mqmq

c.. adapted from RunDec code:
      citations(55)=1
      if (nloop.ge.1) then
         mqpole = mqpole + mqmq*apimq*cf*(1 + (3*lmum)/4.d0)
      elseif (nloop.ge.2) then
         mqpole = mqpole + mqmq*apimq**2*(
     ...
     &        )/2.d0 - (nf*z2)/2.d0) )
      elseif (nloop.eq.3) then
         mqpole = mqpole + mqmq*apimq**3*(cf**3*((495*lmum)/512.d0 - (63
     &        *lmum**2)/128.d0+(9*lmum**3)/128.d0 + (45*lmum*z2)/32.d0 -
     &        (9*lmum*ln2*z2)/4.d0 +(9*lmum*z3)/16.d0) + cf**2*(ca*((
     &        -4219*lmum)/1536.d0 +(21*lmum**2)/64.d0+ (33*lmum**3)
     &        /128.d0 + (49*lmum*z2)/16.d0- (35*lmum*ln2*z2)/8.d0+ (35
     &        *lmum*z3)/32.d0) +   tr*((-9*lmum)/16.d0 -(91*lmum*nf)
     &        /384.d0 -(3*lmum**2*nf)/32.d0 - (3*lmum**3*nf)/32.d0 +(9
     &        *lmum*z2)/8.d0 - (13*lmum*nf*z2)/8.d0 + 2*lmum*ln2*nf*z2
     &        +(lmum*nf*z3)/4.d0))+ cf*(tr**2*((lmum*nf)/2.d0 + (89*lmum
     &        *nf**2)/216.d0 + (13*lmum**2*nf**2)/72.d0 + (lmum**3*nf
     &        **2)/36.d0 -lmum*nf*z2 +(lmum*nf**2*z2)/3.d0) +ca**2
     &        *((13243*lmum)/1728.d0+(2341*lmum**2)/1152.d0 +(121*lmum
     &        **3)/576.d0 - (11*lmum*z2)/12.d0+(11*lmum*ln2*z2)/4.d0-
     &        (11*lmum*z3)/16.d0) + tr*(-(Ca*lmum*ln2*z2) + ca*((-11
     &        *lmum)/8.d0 -(869*lmum*nf)/216.d0 - (373*lmum**2*nf)
     &        /288.d0-(11*lmum**3*nf)/72.d0 + (11*lmum*z2)/4.d0 + lmum
     &        *ln2*z2- (7*lmum*nf*z2)/12.d0 - lmum*ln2*nf*z2 - (lmum*nf
     &        *z3)/2.d0)))
     &        -226.28330594327775 + 28.229231545783556*nf -
     &        0.6526907490815438*nf**2)
      else
...
C-}}}
C-{{{ subroutine polemassinv:

      subroutine polemassinv(mqpole,mu,apimu,nfh,nloop,mqmu)
c..
c..   Computes the MS-bar mass mqmu = mq(mu) from the pole mass mqpole.
c..   apimu = alpha_s(mqmu)/pi.
c..
      implicit real*8 (a-z)
      integer nloop
      include '../commons/common-consts.f'
      ...
      elseif (nloop.eq.1) then
         mqmu = mqpole/( 1 + apimq*cf*(1 + (3*lmum)/4.d0) )
     ...
     &        )/2.d0 - (nf*z2)/2.d0) ) )
         ...
         write(6,*) '<function polemassinv>: nloop = ',nloop
     &        ,' not implemented'
      endif
...
C-}}}
C-{{{ subroutine getmqmq:

      subroutine getmqmq(mqpole,nfh,nloop,mqmq)
c..
c..   Computes the MS-bar mass mqmq = mq(mq) from the pole mass mqpole.
c..
      implicit real*8 (a-z)
      integer nloop,i
      include '../commons/common-consts.f'
      ...
      do i=1,1000
         mu=mqmu
         call runalpha(apimz,mz,mu,nfh,nloop+1,0,apimu)
         call polemassinv(mqpole,mu,apimu,nfh,nloop,mqmu)
         if (dabs(mqmu-mu).lt.1d-5) goto 5001
         if (i.eq.999) call printdie('error in getmqmq')
      enddo
 5001 continue
...
C-{{{ runc5pert:
      ...
      subroutine runc5pert(pseudo,c5sp,log)

      implicit none
      integer pseudo
      real*8 c5sp(0:1,0:3),log
      real*8 beta0,beta1,beta2,beta3
      common /bfunc/ beta0,beta1,beta2,beta3

      if (pseudo.eq.0) then
         !first c5sp(0,3) to avoid corrected (0,2) therein
         c5sp(0,3) = c5sp(0,3) + log**2*beta0*(beta0*c5sp(0,1)-beta1
     &        *c5sp(0,0)) + log*2*(beta0*c5sp(0,2)-beta2*c5sp(0,0))
         c5sp(0,2) = c5sp(0,2) + log*(beta0*c5sp(0,1)-beta1*c5sp(0,0))
      endif
...
C-{{{ runc5:
      ...
      subroutine runc5(pseudo,order,apimz,mz,c5sp,scalein,scaleout)

      implicit none
      integer pseudo,i,order
      real*8 c5sp(0:1,0:3),scalein,scaleout,num,deno,apiin,apiout,apimz
     &     ,mz
      real*8 beta(0:3)
      common /bfunc/ beta

      if (pseudo.eq.1) return

      call runalpha(apimz,mz,scalein,5.d0,order,0,apiin)
      call runalpha(apimz,mz,scaleout,5.d0,order,0,apiout)
      num   = beta(0)
      deno = beta(0)
      do i=1,order
         num  = num  + apiout**i*beta(i)
         deno = deno + apiin**i*beta(i)
      enddo
      c5sp(0,0) = c5sp(0,0)*num/deno
