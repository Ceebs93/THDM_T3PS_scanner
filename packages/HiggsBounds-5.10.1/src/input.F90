...
!******************************************************************
    ...
    use usefulbits, only: Hneut, Hplus, anyH, Chineut, Chiplus
    implicit none
...
contains
    ...
    subroutine setup_input
        !************************************************************
        ! * if inputmethod='datfile', finds whichanalyses,whichinput,np
        ...
        !************************************************************
        use usefulbits, only: np, ndat, n_additional, theo, effC, effC_Hc, g2, partR, &
                              inputmethod, whichinput, &
                              debug, infile1, infile2, &
                              allocate_hadroncolliderextras_parts, allocate_dataset_parts, &
                              allocate_sqcouplratio_parts, allocate_couplratio_parts, &
                              allocate_couplratio_Hc_parts, fill_pdesc
...
! -
        ...
        allocate (effC(ndat))
        allocate (effC_Hc(ndat))
        call allocate_couplratio_parts(effC)
        call allocate_couplratio_Hc_parts(effC_Hc)
...
!      req= part * req
    ...
    subroutine readthefile(n_stem)
        ...
        !************************************************************
        use usefulbits, only: np, theo, ndat, infile1, effC, &
                              n_additional, BRdirectinput, anyH
        implicit none
        !--------------------------------------input
        integer, intent(in) :: n_stem
        !-----------------------------------internal
        character(LEN=50) :: stem
        integer :: jj, i, j, f, k, ios, x, y, count
        double precision :: nc
        logical :: needed, opt
...
!--NEW TO ALLOW DIFFERENT FILE READINGS OF DMHALL_UNCERTAINTIES.dat
            ...
            if (isoptional(n_stem)) then
                if (get_ncol(stem, .False.) .ne. count_columns(f)) then
                    if (get_ncol(stem, .False.) .eq. np(Hneut) + 1) then
                        readHneut = .True.
                        readHplus = .False.
                        numskip = 0
                    else if (get_ncol(stem, .False.) .eq. np(Hplus) + 1) then
                        readHneut = .False.
                        readHplus = .True.
                ...
                if ((get_ncol(stem, .False.) .ne. count_columns(f)) .and. (get_ncol(stem, .True.) .ne. count_columns(f))) then
                    write (*, *) 'wrong no. columns in input file'
                    write (*, *) 'It had', count_columns(f), 'columns'
                    write (*, *) 'but should have been', get_ncol(stem, .True.)
            ...
            case ('MH_GammaTot')
                ...
                do jj = 1, ndat
                    ...
                    print*,np(Hneut),ubound(theo(jj)%particle(anyH)%M)
                    theo(jj)%particle(anyH)%M(:np(Hneut)) = theo(jj)%particle(x)%M
                    theo(jj)%particle(anyH)%Mc(:np(Hneut)) = theo(jj)%particle(x)%M
                    theo(jj)%particle(anyH)%GammaTot(:np(Hneut)) = theo(jj)%particle(x)%GammaTot
                enddo
            ...
            case ('MHplus_GammaTot')
                ...
                do jj = 1, ndat
                    ...
                    if (np(Hneut) .gt. 0) then
                        theo(jj)%particle(anyH)%M(np(Hneut) + 1:) = theo(jj)%particle(x)%M
                        theo(jj)%particle(anyH)%Mc(np(Hneut) + 1:) = theo(jj)%particle(x)%M
                        theo(jj)%particle(anyH)%GammaTot(np(Hneut) + 1:) = theo(jj)%particle(x)%GammaTot
                    endif
                enddo
            case ('MC_GammaTot')
                x = Chiplus
...
!     enddo
            case ('TEV_1H_hadCS_ratios')
                if (get_ncol(stem, .True.) .eq. count_columns(f)) then
                    do jj = 1, ndat
                    ...
                    do jj = 1, ndat
                        read (f, *) nc, (theo(jj)%tev%XS_hj_ratio(i), i=1, np(Hneut)), &
                            ...
                            (theo(jj)%tev%XS_thj_tchan_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%tev%XS_thj_schan_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%tev%XS_tWhj_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%tev%XS_qq_hjZ_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%tev%XS_gg_hjZ_ratio(i), i=1, np(Hneut))
                    enddo
                endif
                ...
                if (get_ncol(stem, .True.) .eq. count_columns(f)) then
                    ...
                    do jj = 1, ndat
                        read (f, *) nc, (theo(jj)%lhc7%XS_hj_ratio(i), i=1, np(Hneut)), &
                            ...
                            (theo(jj)%lhc7%XS_thj_tchan_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%lhc7%XS_thj_schan_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%lhc7%XS_tWhj_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%lhc7%XS_qq_hjZ_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%lhc7%XS_gg_hjZ_ratio(i), i=1, np(Hneut))
                    enddo
                endif
...
!     enddo
                ...
                if (get_ncol(stem, .True.) .eq. count_columns(f)) then
                    ...
                    do jj = 1, ndat
                        read (f, *) nc, (theo(jj)%lhc8%XS_hj_ratio(i), i=1, np(Hneut)), &
                            ...
                            (theo(jj)%lhc8%XS_thj_tchan_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%lhc8%XS_thj_schan_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%lhc8%XS_tWhj_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%lhc8%XS_qq_hjZ_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%lhc8%XS_gg_hjZ_ratio(i), i=1, np(Hneut))
                    enddo
                endif
                ...
                if (get_ncol(stem, .True.) .eq. count_columns(f)) then
                    ...
                    do jj = 1, ndat
                        read (f, *) nc, (theo(jj)%lhc13%XS_hj_ratio(i), i=1, np(Hneut)), &
                            ...
                            (theo(jj)%lhc13%XS_thj_tchan_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%lhc13%XS_thj_schan_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%lhc13%XS_tWhj_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%lhc13%XS_qq_hjZ_ratio(i), i=1, np(Hneut)), &
                            (theo(jj)%lhc13%XS_gg_hjZ_ratio(i), i=1, np(Hneut))
                    enddo
                endif
            case ('BR_H_OP')
                do jj = 1, ndat
            ...
            case ('TEV_Hplus_hadCS')
                do jj = 1, ndat
                    read (f, *) nc, (theo(jj)%tev%XS_Hpmjtb(i), i=1, np(Hplus)), &
                        (theo(jj)%tev%XS_Hpmjcb(i), i=1, np(Hplus)), &
                        (theo(jj)%tev%XS_Hpmjbjet(i), i=1, np(Hplus)), &
                        (theo(jj)%tev%XS_Hpmjcjet(i), i=1, np(Hplus)), &
                        (theo(jj)%tev%XS_qq_Hpmj(i), i=1, np(Hplus)), &
                        (theo(jj)%tev%XS_HpmjW(i), i=1, np(Hplus)), &
                        (theo(jj)%tev%XS_HpmjZ(i), i=1, np(Hplus)), &
                        (theo(jj)%tev%XS_vbf_Hpmj(i), i=1, np(Hplus)), &
                        (theo(jj)%tev%XS_HpjHmj(i), i=1, np(Hplus)), &
                        ((theo(jj)%tev%XS_Hpmjhi(j, i), i=1, np(Hneut)), j=1, np(Hplus))
                enddo
            case ('LHC7_Hplus_hadCS')
                do jj = 1, ndat
                    read (f, *) nc, (theo(jj)%lhc7%XS_Hpmjtb(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc7%XS_Hpmjcb(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc7%XS_Hpmjbjet(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc7%XS_Hpmjcjet(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc7%XS_qq_Hpmj(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc7%XS_HpmjW(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc7%XS_HpmjZ(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc7%XS_vbf_Hpmj(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc7%XS_HpjHmj(i), i=1, np(Hplus)), &
                        ((theo(jj)%lhc7%XS_Hpmjhi(j, i), i=1, np(Hneut)), j=1, np(Hplus))
                enddo
            case ('LHC8_Hplus_hadCS')
                do jj = 1, ndat
                    read (f, *) nc, (theo(jj)%lhc8%XS_Hpmjtb(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc8%XS_Hpmjcb(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc8%XS_Hpmjbjet(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc8%XS_Hpmjcjet(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc8%XS_qq_Hpmj(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc8%XS_HpmjW(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc8%XS_HpmjZ(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc8%XS_vbf_Hpmj(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc8%XS_HpjHmj(i), i=1, np(Hplus)), &
                        ((theo(jj)%lhc8%XS_Hpmjhi(j, i), i=1, np(Hneut)), j=1, np(Hplus))
                enddo
            case ('LHC13_Hplus_hadCS')
                do jj = 1, ndat
                    read (f, *) nc, (theo(jj)%lhc13%XS_Hpmjtb(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc13%XS_Hpmjcb(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc13%XS_Hpmjbjet(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc13%XS_Hpmjcjet(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc13%XS_qq_Hpmj(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc13%XS_HpmjW(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc13%XS_HpmjZ(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc13%XS_vbf_Hpmj(i), i=1, np(Hplus)), &
                        (theo(jj)%lhc13%XS_HpjHmj(i), i=1, np(Hplus)), &
                        ((theo(jj)%lhc13%XS_Hpmjhi(j, i), i=1, np(Hneut)), j=1, np(Hplus))
                enddo
            case ('BR_H_NP')
...
!  integer, intent(out) :: ii
    ...
    subroutine command_line_how2
        ...
        if (.not. official) then
            if (ubound(np, dim=1) .eq. 4) then
                ...
                write (*, *) 'e.g.'
                write (*, *) ' ./HiggsBounds LandH hadr 3 1 4 2 mhmax'
            else
    ...
    subroutine check_number_of_particles
        ...
        ! write(*,*) "Calling check_number_of_particles."
        ...
        if ((inputmethod .eq. 'datfile') .and. (official)) then
            do x = 3, 4 !ubound(np, dim=1) !i.e. particles other than Hneut,Hplus
                if (np(x) .gt. 0) then
