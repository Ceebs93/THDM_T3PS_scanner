...
contains
    ...
    !********************************************************
    function get_SPhenocrosssectionCMenergy(particlePDGcode_1, particlePDGcode_2, pol_1, pol_2)
        !get the centre of mass of the SPheno cross section
        !********************************************************
        use usefulbits, only: vsmall
        implicit none
        integer, intent(in) :: particlePDGcode_1, particlePDGcode_2
        ...
        if (minval(blocklines) .le. 0) then
            ...
            elseif (abs(pol_1 - pol_1_fromblock) .gt. vsmall) then
                stop 'problem in function get_SPhenocrosssectionCMenergy(4)'
            elseif (abs(pol_2 - pol_2_fromblock) .gt. vsmall) then
