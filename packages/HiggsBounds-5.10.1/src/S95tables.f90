...
!******************************************************************
    use S95tables_type1
    use S95tables_type2
    use usefulbits, only: Hneut, Hplus, anyH, BR_hjdijet, BR_Hpjdijet
    implicit none
    private
    public :: S95_t1, S95_t2, &
...
contains
    ...
    subroutine setup_S95tables
        ...
        ! or taken away:
        ntable1 = 235
        ntable2 = 48              ! table type 2 involves 2 variables
...
!    stop 'need to extend lower range of lhc8XS_SM_functions'
        ...
        if (Exptrange_Mhmax_forSMXS(get_collider_element_number('LHC13')) .gt. lhc13XS_SM_functions_xmax) then
            write (*, *) 'current max mass for lhc13XS_SM_functions is', lhc13XS_SM_functions_xmax
...
!  if(predobs.eq.1) write(*,*) 'Interpolated value = ', interpol
    ...
    subroutine calcfact_t1(c, jj, t, cfact_t1, M_av, nc)
        ...
        !**********************************************************
        use usefulbits, only: dataset, np, div, vvsmall, mtau, small
        ...
        double precision :: BR_Whad, BR_Zhad, BR_doubleratio
        ...
        allocate (fact(npart), model_like(npart), particle_type(npart), mass(npart))
        mass = t%particle(S95_t1(c)%particle_x)%M(:)
        if (S95_t1(c)%particle_x .eq. anyH) then
            particle_type(:np(Hneut)) = Hneut
            if (np(Hplus) .gt. 0) then
                particle_type(np(Hneut) + 1:) = Hplus
            end if
        else
            particle_type = S95_t1(c)%particle_x
        end if
        ...
        do j = 1, npart
            jc = j
            if (S95_t1(c)%particle_x .eq. anyH .and. particle_type(j) .eq. Hplus) then
                jc = j - np(Hneut)
            end if
            ...
            if ((abs(mass(jj) - mass(j)) .le. S95_t1(c)%deltax) &
                ...
                case (5757)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                case (8957, 5472, 9219, 9463, 9596, 5828, 1970, 3493, 5972, 3155, &
                      9868, 10068, 6092, 10217, 10239, 10796, 6220)
                ...
                case (1514, 5601, 5737)
                    fact(j) = (t%tev%XS_hjZ_ratio(j)*t%tev%XS_HZ_SM(j) &
                              ...
                              *t%BR_hjgaga(j)
                case (5858, 6177, 6295, 10065, 4960)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                case (7081, 9166, 9483, 5586, 9642, 1266, 0432, 9891, 5285, 3935, 6087, &
                      6170, 10212, 6223, 6299, 10583, 10798, 10596)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                case (2012015)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                ...
                case (1269, 1270, 2011094, 13035, 160311, 18021)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                case (1811)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                case (1812, 2011138, 2011151, 2760, 2013090, 2014050, 14020, 11002, 11008, 79152)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                case (6008, 9998)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                ...
                case (6091, 1268)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                case (2011048, 11004, 11015, 11013, 11028, 11006, 11017, 110271, 110272, &
                      14161, 14162, 5064, 2012017, 2011150, 2011131)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                ...
                case (110212)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                case (17013, 2011025, 2011085, 2011161, 5895, 1414, 2012091, 2012168, &
                      1487, 12001, 12015, 13001, 11010, 11030, 11021)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
...
!       print *, 1000.D0*t%lhc8%XS_hjZ_ratio(j)*t%lhc8%XS_HZ_SM(j)*(BR_Zll+BR_Ztautau), fact(j)
                ...
                case (2749, 12191)
                    fact(j) = t%lhc13%XS_bb_hj_ratio(j)*t%lhc13%XS_bb_H_SM(j)*t%BR_hjbb(j)
                case (1810118221, 1810118222)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                case (6583)
!    Data given in fb - (multiply by 1000)
! (TS 2016/10/13: Added conservatively estimated signal efficiency factor of ~0.56)
                ...
                case (7355)
                    do ii = 1, npart
                        ...
                        if (abs(t%particle(Hneut)%M(ii) - 125.0D0) .lt. (10.0D0 + t%particle(Hneut)%dMh(ii))) then
                            fact_tmp = t%lhc13%XS_hjZ_ratio(ii)*t%lhc13%XS_HZ_SM(ii) &
                                       + t%lhc13%XS_hjW_ratio(ii)*t%lhc13%XS_HW_SM(ii)
                ...
                case (10191)
                    do ii = 1, npart
                        fact_tmp = 0.0D0
                        if (abs(t%particle(Hneut)%M(ii) - 125.0D0) .lt. (10.0D0 + t%particle(Hneut)%dMh(ii))) then
                            ! Limit is given in % (therefore, multiply prediction by 100.0)
                            ! Multiply by symmetry factor 2 for unidentical final states.
                            fact(j) = fact(j) + t%lhc13%XS_hj_ratio(ii)*t%BR_hkhjhi(ii, j, j)*2.0D0 &
                                      *t%BR_hjbb(j)*t%BR_hjtautau(j)*100.0D0
                        end if
                    end do
                ...
                case (18006)
                    ! Test whether assumption on hj->mumu vs hj->tautau BR is fulfilled (CP-odd Higgs-like BR ratios):
                    if ((t%particle(Hneut)%M(j) .gt. (2.0D0*mtau)) .and. (t%BR_hjtautau(j) .gt. 0.0D0)) then
                        BR_doubleratio = t%BR_hjmumu(j)/t%BR_hjtautau(j)/(t%BR_Hmumu_SM(j)/t%BR_Htautau_SM(j))
                        ! Maximally allowed deviation from assumption ~ 5%
                        if (abs(BR_doubleratio - (1.0D0 - 4.0D0*(mtau/t%particle(Hneut)%M(j))**2.0D0)) &
                            /BR_doubleratio .lt. 0.05D0) then
                            do ii = 1, npart
                                fact_tmp = 0.0D0
                                if (abs(t%particle(Hneut)%M(ii) - 125.0D0) .lt. (10.0D0 + t%particle(Hneut)%dMh(ii))) then
                                    call model_likeness(ii, S95_t1(c)%id, t, model_like(ii), fact_tmp)
                ...
                case (17020322, 539)
                    do ii = 1, npart
                        ...
                        if (abs(t%particle(Hneut)%M(ii) - 125.0D0) .lt. (25.0D0 + t%particle(Hneut)%dMh(ii))) then
                            call model_likeness(ii, S95_t1(c)%id, t, model_like(ii), fact_tmp)
                            do iii = 1, npart ! Consider additional light near-by Higgs bosons (to h_j)
                                if (abs(t%particle(Hneut)%M(j) - t%particle(Hneut)%M(iii)) .le. &
                                    (2.5D0 + t%particle(Hneut)%dMh(j) + t%particle(Hneut)%dMh(iii))) then
                                    if (j .eq. iii) then
                                        combfactor = 0.5D0
                                    else
                                        combfactor = 1.0D0
                                    end if
                                    fact(j) = fact(j) + fact_tmp*combfactor*t%BR_hkhjhi(ii, j, iii)* &
                                              (t%BR_hjmumu(j)*t%BR_hjbb(iii) + t%BR_hjmumu(iii)*t%BR_hjbb(j))
                                end if
                            end do
                        end if
                    end do
                case (6359)
                    do ii = 1, npart
                        fact_tmp = 0.0D0
                        if (abs(t%particle(Hneut)%M(ii) - 125.0D0) .lt. (10.0D0 + t%particle(Hneut)%dMh(ii))) then
                            fact_tmp = (t%lhc13%XS_hj_ratio(ii)*t%lhc13%XS_H_SM(ii) + &
                                        t%lhc13%XS_vbf_ratio(ii)*t%lhc13%XS_vbf_SM(ii))
                            do iii = 1, npart ! Consider additional light near-by Higgs bosons (to h_j)
                                if (abs(t%particle(Hneut)%M(j) - t%particle(Hneut)%M(iii)) .le. &
                                    (2.5D0 + t%particle(Hneut)%dMh(j) + t%particle(Hneut)%dMh(iii))) then
                                    if (j .eq. iii) then
                ...
                case (17020323, 4865)
                    do ii = 1, npart
                        ...
                        if (abs(t%particle(Hneut)%M(ii) - 125.0D0) .lt. (25.0D0 + t%particle(Hneut)%dMh(ii))) then
                            call model_likeness(ii, S95_t1(c)%id, t, model_like(ii), fact_tmp)
                            ! Check assumption BR(mumu)/BR(tautau) \simeq m(mu)^2/m(tau)^2 (allow for 10% deviation):
                            if (abs(1.777D0**2.0*t%BR_hjmumu(j) - 0.106D0**2.0*t%BR_hjtautau(j)) .le. &
                                (0.1D0*0.106D0**2.0*t%BR_hjtautau(j))) then
                                do iii = 1, npart ! Consider additional light near-by Higgs bosons (to h_j)
                                    ! Check for nearby light Higgs bosons
                                    if (abs(t%particle(Hneut)%M(j) - t%particle(Hneut)%M(iii)) .le. &
                                        (2.5D0 + t%particle(Hneut)%dMh(j) + t%particle(Hneut)%dMh(iii))) then
                                        if (j .eq. iii) then
                                            combfactor = 0.5D0
                                        ...
                                        fact(j) = fact(j) + fact_tmp*combfactor*t%BR_hkhjhi(ii, j, iii)* &
                                                  (t%BR_hjmumu(j)*t%BR_hjtautau(iii) + t%BR_hjmumu(iii)*t%BR_hjtautau(j))
                                    end if
                                end do
                            end if
                        end if
                ...
                case (011811)
                    ...
                    do ii = 1, npart
                        ...
                        if (abs(t%particle(Hneut)%M(ii) - 125.0D0) .lt. (10.0D0 + t%particle(Hneut)%dMh(ii)) .and. &
                            abs(t%particle(Hneut)%M(iii) - 125.0D0) .lt. (10.0D0 + t%particle(Hneut)%dMh(iii))) then
                ...
                case (18013)
                    ! Limit given in pb
                    do ii = 1, npart
                        do iii = 1, npart ! No symmetry factor (1/2) because limit is set on BR(H->hh->bbZZ)
                        if (abs(t%particle(Hneut)%M(ii) - 125.0D0) .lt. (10.0D0 + t%particle(Hneut)%dMh(ii)) .and. &
                            abs(t%particle(Hneut)%M(iii) - 125.0D0) .lt. (10.0D0 + t%particle(Hneut)%dMh(iii))) then
                            fact(j) = fact(j) + t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j) &
                                      *t%BR_hkhjhi(j, ii, iii)*(t%BR_hjZZ(ii)*t%BR_hjbb(iii))
                        end if
                        end do
                    end do
                case (16002, 2016049)
                    ...
                    do ii = 1, npart
                        ...
                        if (abs(t%particle(Hneut)%M(ii) - 125.0D0) .lt. (10.0D0 + t%particle(Hneut)%dMh(ii)) .and. &
                            abs(t%particle(Hneut)%M(iii) - 125.0D0) .lt. (10.0D0 + t%particle(Hneut)%dMh(iii))) then
                            fact(j) = fact(j) + 0.5D0*1000.0D0*t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j) &
                ...
                case (14013)
                    ...
                    do ii = 1, npart
                        ...
                        if (abs(t%particle(Hneut)%M(ii) - 125.0D0) .lt. (10.0D0 + t%particle(Hneut)%dMh(ii)) .and. &
                            abs(t%particle(Hneut)%M(iii) - 125.0D0) .lt. (10.0D0 + t%particle(Hneut)%dMh(iii))) then
                            fact(j) = fact(j) + 0.5D0*1000.0D0*t%lhc8%XS_hj_ratio(j)*t%lhc8%XS_H_SM(j) &
                                      *t%BR_hkhjhi(j, ii, iii)*t%BR_hjbb(iii)*t%BR_hjbb(ii)
                        end if
                        end do
                    end do
                case (110281)
                    ...
                    do ii = 1, npart
                        ...
                        if (abs(t%particle(Hneut)%M(ii) - 125.0D0) .lt. (10.0D0 + t%particle(Hneut)%dMh(ii)) .and. &
                            abs(t%particle(Hneut)%M(iii) - 125.0D0) .lt. (10.0D0 + t%particle(Hneut)%dMh(iii))) then
                            fact(j) = fact(j) + 0.5D0*t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j) &
                                      *t%BR_hkhjhi(j, ii, iii)*t%BR_hjWW(iii)*t%BR_hjWW(ii) &
                                      /(t%BR_HWW_SM(iii)*t%BR_HWW_SM(ii))
                        end if
                        end do
                    end do
                case (044781)
                ...
                case (180051, 1712065181, 2020043)
                    ! Limit given in pb (Z->ll unfolded)
                    do ii = 1, npart
                        if (abs(t%particle(Hneut)%M(ii) - 125.0D0) .lt. 10.0D0) then
                ...
                case (2018025)
                    ...
                    fact(j) = 1000.0D0*( &
                              ...
                              t%BR_hjgaga(j)
                case (4968)
                    fact(j) = t%lhc13%XS_tthj_ratio(j)*t%lhc13%XS_ttH_SM(j)*t%BR_hjmumu(j)
                case (1908064631)
                    if (t%CP_value(j) .eq. 1 .and. t%br_hjWW(j) + t%br_hjZZ(j) .lt. small) then
                        fact(j) = (t%lhc13%XS_tthj_ratio(j)*t%lhc13%XS_ttH_SM(j) + &
                                   t%lhc13%XS_thj_schan_ratio(j)*t%lhc13%XS_tH_schan_SM(j) + &
                                   t%lhc13%XS_thj_tchan_ratio(j)*t%lhc13%XS_tH_tchan_SM(j))* &
                                  t%BR_hjtt(j)*1D3
                    else
                        fact(j) = 0D0
                    end if
                ...
                case (20160741, 2016062, 170332)
                    fact(j) = t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j)*t%BR_hjWW(j)
                case (20160742)
                    fact(j) = t%lhc13%XS_vbf_ratio(j)*t%lhc13%XS_vbf_SM(j)*t%BR_hjWW(j)
                case (23801)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                    if (model_like(j) .eq. 1) then
                        ! Limit is given in fb.
                        fact(j) = 1000.0D0*t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j)*(t%BR_hjWW(j) + t%BR_hjZZ(j))
                    end if
                case (23802)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                    if (model_like(j) .eq. 1) then
                        ! Limit is given in fb.
                        fact(j) = 1000.0D0*t%lhc13%XS_vbf_ratio(j)*t%lhc13%XS_vbf_SM(j)*(t%BR_hjWW(j) + t%BR_hjZZ(j))
                    end if
                case (170331)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                    if (model_like(j) .eq. 1) then
                        ! Limit is given in pb.
                        fact(j) = (t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j) + &
                                   t%lhc13%XS_vbf_ratio(j)*t%lhc13%XS_vbf_SM(j))*t%BR_hjWW(j)
                    end if
                case (2011112)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                case (2011135)
                ...
                case (6224, 6225, 6226)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                case (04670, 046701, 046702, 17030, 2025, 4873, 8567, 336)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                case (0038911, 0038912, 0038913, 0038914)
                    call model_likeness(j, S95_t1(c)%id, t, model_like(j), fact(j))
                case (160312, 79151, 18014)
                    !    Data given in pb
                    fact(j) = t%lhc13%XS_Hpmjtb(j)*t%BR_Hpjtaunu(j)
                case (1504233)
                    !    Data given in fb
                    fact(j) = 1000.0D0*t%lhc8%XS_vbf_Hpmj(j)*t%BR_HpjWZ(j)
                case (2016089, 3599, 18015, 2020039, 2002113251)
                    !    Data given in pb
                    fact(j) = t%lhc13%XS_Hpmjtb(j)*t%BR_Hpjtb(j)
                case (2016025)
                    fact(j) = t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j)*t%BR_hjbb(j)
                case (180509299)
                    ! different acceptances for qq-induced and gg-induced kinematics
                    fact(j) = (t%lhc13%XS_gg_hj_ratio(j)*t%lhc13%XS_gg_H_SM(j)* &
                               (3.927043D-1 + 1.379542D-5*(mass(j) - 1.25D3) + 4.185258D+6/(999D0 - mass(j))**3) & ! acceptance gg
                               + t%lhc13%XS_bb_hj_ratio(j)*t%lhc13%XS_bb_H_SM(j)* &
                               (4.142112D-1 + 6.344066D-6*(mass(j) - 1.25D3) + 4.549180D+6/(999D0 - mass(j))**3)) & ! acceptance qq
                              *t%BR_hjbb(j)* &
                              (0.630374 + 0.060125*mass(j)/1D3 - 0.278384*log(mass(j)/1D3)) ! efficiency
                case (2016044)
                ...
                case (31521, 81441)
                    fact(j) = 1000.0D0*t%lhc13%XS_gg_hj_ratio(j)*t%lhc13%XS_gg_H_SM(j)*t%BR_hjmumu(j)
                case (31522, 81442)
                    fact(j) = 1000.0D0*t%lhc13%XS_bb_hj_ratio(j)*t%lhc13%XS_bb_H_SM(j)*t%BR_hjmumu(j)
                case (180171)
                    fact(j) = t%lhc13%XS_gg_hj_ratio(j)*t%lhc13%XS_gg_H_SM(j)*t%BR_hjmutau(j)
                case (1807065734)
                    fact(j) = t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j)*t%BR_hjmutau(j)
                case (180172)
                    fact(j) = t%lhc13%XS_gg_hj_ratio(j)*t%lhc13%XS_gg_H_SM(j)*t%BR_hjetau(j)
                case (1807065733)
                    fact(j) = t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j)*t%BR_hjetau(j)
                case (61311)
                    fact(j) = t%lhc13%XS_hj_ratio(j)*t%BR_hjetau(j)
                case (61312)
                    fact(j) = t%lhc13%XS_hj_ratio(j)*t%BR_hjmutau(j)
                case (102351)
                    fact(j) = t%lhc13%XS_hj_ratio(j)*t%BR_hjemu(j)
                case (180201122)
                    fact(j) = 1000.0D0*t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j)*t%BR_hjemu(j)
                case (1807065731)
                    fact(j) = t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j)*t%BR_hjemu(j)
                case (1807065732)
                    fact(j) = t%lhc13%XS_gg_hj_ratio(j)*t%lhc13%XS_gg_H_SM(j)*t%BR_hjemu(j)
                case default
        ...
        if (fact(jj) .le. vvsmall) then!A  !Higgs jj doesn't contribute - wait until another call of this subroutine before
            ...
            elseif (S95_t1(c)%particle_x .ne. Hneut) then!B
                ...
                case (8742, 5482, 5570, 4493, 9475, 5876, 1024, 9889, 3534, 6089, 10235, 3047, &
                      10799, 3564, 6166, 6296)
                    do j = 1, npart
                ...
                case (8957, 5472, 9219, 9463, 9596, 5828, 1970, 3493, 5972, 3155, 9868, 10068, &
                      6092, 10217, 10239, 10796, 6220)
                    do j = 1, npart
                ...
                case (7081, 9166, 9483, 5586, 9642, 1266, 0432, 9891, 5285, 3935, 6087, 6170, 10212, 6223, &
                      6299, 10583, 10798, 10596)
                case (2012015)
                case (9248, 5845, 4800, 5858, 6177, 6295, 10065, 10133, 10439, 4960)
                ...
                case (2011048, 11004, 11015, 11013, 11028, 11006, 11017, 110271, 110272, 14161, 14162, &
                      5064, 2012017, 2011150, 2011131)
                case (2011162, 1415, 2012092, 20130131)
                case (11025, 1997, 12041, 130021, 130022, 009361, 16034)
                case (2011026, 11005, 11016, 11026, 3478, 3357, 2011148, 2012016)
                case (110212)
                case (17013, 2011025, 2011085, 2011161, 5895, 1414, 2012091, 2012168, 1487, 12001, &
                      12015, 13001, 11010, 11030, 11021)
                case (13006, 13075515, 2013009, 3051)
                case (11031, 12044, 13012)
                case (13011)
                case (11034, 12039, 13009, 2012078, 12006, 12051)
                ...
                case (13022)
                case (13013)
                case (13441, 13442, 13443, 6682, 5105, 59371, 59372)
                case (13018)
                case (2013011, 3244)
                case (2011112)
                case (6583, 14006, 14037, 14031, 2018025)
                case (2011135)
                case (1508329, 2015080, 2749, 12191, 1810118221, 1810118222)
                case (17020321, 18006, 17020322, 6359, 17020323, 4865, 539, 7355, 10191)
                case (6224, 6225, 6226)
                case (04670, 046701, 046702, 17030, 2025, 4873, 8567, 336)
                case (0038911, 0038912, 0038913, 0038914)
                case (160312, 18014, 2016089, 3599, 18015, 2020039, 2002113251, 14011, 011811, 16029, 17002, 14013, 110281, 18013)
                case (16002, 011812, 20160851, 20160852, 20170501, 20170502, 79151)
                case (1504233, 1506534, 5051)
                case (160371, 160372, 044781, 044782, 180051, 180052, 1712065181, 1712065182, 2020043, 17006)
                case (20160741, 20160742, 2016062, 170332, 20160821, 20160822, 20160823, 20160792, 20160793, 2016049, 063861)
                case (2016025, 180509299, 2016044, 201608391, 1604833, 2016056, 20160551, 20160552, 15009)
                case (31521, 81441, 31522, 81442)
                case (180171, 1807065734, 180172, 1807065733, 61311, 61312, 102351, 180201122, 1807065731, 1807065732)
                case (16030, 23801, 23802, 170331)
                case (4968)
                case (1908064631, 1908064632)
                case default
    ...
    subroutine calcfact_t2(c, jj, ii, t, cfact_t2, axis_i, axis_j, nc)
        ...
        !**********************************************************
        use usefulbits, only: dataset, np, vsmall, not_a_particle, extrapolatewidth, effC, effC_Hc
        use theory_XS_SM_functions, only: neutral_scalar_plus_photon_lhc13, positive_charged_scalar_plus_photon_lhc13, &
                                          negative_charged_scalar_plus_photon_lhc13
        ! n.b.: make use of effC and effC_Hc directly. This only works in subroutine mode with
        !       only one dataset given. A more general implementation would copy the effective couplings
        !       to the dataset instance theo
        implicit none
        !--------------------------------------input
        type(dataset) :: t
        integer :: c, jj, ii
        ...
        integer :: nc
        !-------------------------------------------
        integer :: f, i, j, npart2, npart1, iii, jjj
        double precision :: fact, eps2, crosssection, Mi_av, Mj_av, masstot, BR_Zll, acceptance
        double precision :: coupling, couplingL, couplingR, gammatot
        double precision :: posRate, negRate
        double precision, allocatable :: massj(:), massi(:), mass_rec(:), final_state_rate(:), &
            final_state_rate_pos(:), final_state_rate_neg(:)
        integer, allocatable :: particle_type(:)
        ...
        allocate (massj(npart2), particle_type(npart2))
        massj = t%particle(S95_t2(c)%particle_x2)%M(:)
        if (S95_t2(c)%particle_x2 .eq. anyH) then
            particle_type(:np(Hneut)) = Hneut
            if (np(Hplus) .gt. 0) then
                particle_type(np(Hneut) + 1:) = Hplus
            end if
        else
            particle_type = S95_t2(c)%particle_x2
        end if
...
!             &      t%BR_hjbb(i) * t%BR_hjbb(i) )
        ...
        case (110282)
            ! Limit given in pb
            fact = test_appl(t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j)*t%BR_hjhihi(j, i)* &
                             t%BR_hjWW(i)**2.0D0/t%BR_HWW_SM(i)**2.0D0)
        case (150011)
            ! Limit given in fb, multiply by 1000.
            fact = test_appl(1000.0D0*t%lhc8%XS_hj_ratio(j)*t%lhc8%XS_H_SM(j)*t%BR_hjhiZ(j, i)* &
                             t%BR_hjtautau(i)*BR_Zll)
        ...
        case (170271)
            ! Approximate CP-odd Higgs-top-top coupling from partial width / SM, include kinematic factor.
!          write(*,*) "170271, model " ,j, t%BR_hjtt(j)*t%particle(S95_t2(c)%particle_x2)%GammaTot(j), Mj_av
!          write(*,*) "170271 SM     " ,j, t%BR_Htt_SM(j)*t%GammaTot_SM(j)
!          fact = test_appl( sqrt( t%BR_hjtt(j)*t%particle(S95_t2(c)%particle_x2)%GammaTot(j) / &
!          &                      (t%BR_Htt_SM(j)*t%GammaTot_SM(j)) * (1.0D0 - 4.0D0 * (mt/Mj_av)**2.0D0)) )
!n.b. the approximation through BRs is dangerous, as the providing spectrum generator may use
!     different SM BR values than HiggsBounds, thus introducing an unwanted offset, and in particular
!     then causing the comparison test with the SM-normalized gluon-fusion cross section to fail.
!     Use instead the SM-normalized ttH cross section here (explicit CP test is anyways included):
            fact = test_appl(sqrt(t%lhc13%XS_tthj_ratio(j)))
        case (170272)
            ! Approximate CP-even Higgs-top-top coupling from partial width / SM.
!          fact = test_appl( sqrt( t%BR_hjtt(j)*t%particle(S95_t2(c)%particle_x2)%GammaTot(j) / &
!          &                      (t%BR_Htt_SM(j)*t%GammaTot_SM(j))) )
!n.b. the approximation through BRs is dangerous, as the providing spectrum generator may use
!     different SM BR values than HiggsBounds, thus introducing an unwanted offset, and in particular
!     then causing the comparison test with the SM-normalized gluon-fusion cross section to fail.
!     Use instead the SM-normalized ttH cross section here (explicit CP test is anyways included):
            fact = test_appl(sqrt(t%lhc13%XS_tthj_ratio(j)))
! TS, 2020-11-02, implementation of ATLAS dijet+photon search
        case (109171, 109172)
            f = 0
            allocate (mass_rec(npart2))
            do j = 1, npart2
                if ((abs(massj(jj) - massj(j)) .le. S95_t2(c)%deltax)) then
                    ! &                  .and. (massj(jj) .le. massj(j))) then
                    crosssection = 0.0D0
                    if (particle_type(j) .eq. Hneut) then
                        jjj = j
                        do iii = 1, 9 ! Loop over relevant couplings
                            select case (iii) ! Select relevant coupling strength
                                ! n.b. make use effC and effC_Hc directly. See comments at beginning of subroutine.
                            case (1)
                                coupling = sqrt(effC(1)%hjuu_s(j)**2.0D0 + effC(1)%hjuu_p(j)**2.0D0)
                            case (2)
                                coupling = sqrt(effC(1)%hjdd_s(j)**2.0D0 + effC(1)%hjdd_p(j)**2.0D0)
                            case (3)
                                coupling = sqrt(effC(1)%hjcc_s(j)**2.0D0 + effC(1)%hjcc_p(j)**2.0D0)
                            case (4)
                                coupling = sqrt(effC(1)%hjss_s(j)**2.0D0 + effC(1)%hjss_p(j)**2.0D0)
                            case (5)
                                coupling = sqrt(effC(1)%hjbb_s(j)**2.0D0 + effC(1)%hjbb_p(j)**2.0D0)
                            case (6)
                                coupling = sqrt(effC(1)%hjuc_s(j)**2.0D0 + effC(1)%hjuc_p(j)**2.0D0)
                            case (7)
                                coupling = sqrt(effC(1)%hjds_s(j)**2.0D0 + effC(1)%hjds_p(j)**2.0D0)
                            case (8)
                                coupling = sqrt(effC(1)%hjdb_s(j)**2.0D0 + effC(1)%hjdb_p(j)**2.0D0)
                            case (9)
                                coupling = sqrt(effC(1)%hjsb_s(j)**2.0D0 + effC(1)%hjsb_p(j)**2.0D0)
                            end select

                            crosssection = crosssection + &
                                           neutral_scalar_plus_photon_lhc13(massj(j), coupling, iii, .True.)* &
                                           acceptance_type2(109171, massj(j), 0.0D0, Hneut, 0, 0, iii) ! acceptance before mjj-cut
                        end do
                        select case (S95_t2(c)%id)
                        case (109171)
                            allocate (final_state_rate(9))
                            mass_rec(j) = 0.0D0
                            do iii = 1, 9 ! Loop over partons for the final state
                                select case (iii)
                                case (1)
                                    final_state_rate(iii) = t%BR_hjuu(j)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hneut, 0, 1, iii)
                                case (2)
                                    final_state_rate(iii) = t%BR_hjdd(j)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hneut, 0, 1, iii)
                                case (3)
                                    final_state_rate(iii) = t%BR_hjcc(j)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hneut, 0, 1, iii)
                                case (4)
                                    final_state_rate(iii) = t%BR_hjss(j)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hneut, 0, 1, iii)
                                case (5)
                                    final_state_rate(iii) = t%BR_hjbb(j)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hneut, 0, 1, iii)
                                case (6)
                                    final_state_rate(iii) = t%BR_hjuc(j)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hneut, 0, 1, iii)
                                case (7)
                                    final_state_rate(iii) = t%BR_hjds(j)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hneut, 0, 1, iii)
                                case (8)
                                    final_state_rate(iii) = t%BR_hjdb(j)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hneut, 0, 1, iii)
                                case (9)
                                    final_state_rate(iii) = t%BR_hjsb(j)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hneut, 0, 1, iii)
                                end select
                                mass_rec(j) = mass_rec(j) + final_state_rate(iii)* &
                                              shifted_mass(S95_t2(c)%id, massj(j), particle_type(j), iii)
                            end do
                            mass_rec(j) = mass_rec(j)/sum(final_state_rate)
                            crosssection = crosssection*sum(final_state_rate)
                            deallocate (final_state_rate)
                            ! acceptances after mjj-cut (dependent on final state)
                        case (109172)
                            crosssection = crosssection* &
                                           t%BR_hjbb(j)*acceptance_type2(109171, massj(j), 0.0D0, Hneut, 0, 1, 5)* &
                                           acceptance_type2(109172, massj(j), 0.0D0, Hneut, 0, 0, 5) ! b-tagging acceptance
                            mass_rec(j) = shifted_mass(S95_t2(c)%id, massj(j), Hneut, 5)
                        end select

                    else if (particle_type(j) .eq. Hplus .and. (S95_t2(c)%id .eq. 109171)) then ! Charged scalar contribution only to inclusive search
                        posRate = 0D0
                        negRate = 0D0
                        jjj = j - np(Hneut) ! Charged Higgs boson index
                        do iii = 1, 6 ! Loop over relevant couplings
                            ! M = shifted_mass(S95_t2(c)%id, massj(j), particle_type(j), iii) ! Reconstructed mass (dependent on couplings)
                            select case (iii) ! Select relevant coupling strength
                                ! n.b. make use effC and effC_Hc directly. See comments at beginning of subroutine.
                            case (1)
                                couplingL = effC_Hc(1)%hcjud_L(jjj)
                                couplingR = effC_Hc(1)%hcjud_R(jjj)
                            case (2)
                                couplingL = effC_Hc(1)%hcjcs_L(jjj)
                                couplingR = effC_Hc(1)%hcjcs_R(jjj)
                            case (3)
                                couplingL = effC_Hc(1)%hcjus_L(jjj)
                                couplingR = effC_Hc(1)%hcjus_R(jjj)
                            case (4)
                                couplingL = effC_Hc(1)%hcjcd_L(jjj)
                                couplingR = effC_Hc(1)%hcjcd_R(jjj)
                            case (5)
                                couplingL = effC_Hc(1)%hcjub_L(jjj)
                                couplingR = effC_Hc(1)%hcjub_R(jjj)
                            case (6)
                                couplingL = effC_Hc(1)%hcjcb_L(jjj)
                                couplingR = effC_Hc(1)%hcjcb_R(jjj)
                            end select
                            posRate = posRate + &
                                      positive_charged_scalar_plus_photon_lhc13(massj(j), couplingL, couplingR, iii, .True.)* &
                                      acceptance_type2(109171, massj(j), 0.0D0, Hplus, 0, 0, iii, +1) ! acceptance before mjj-cut
                            negRate = negRate + &
                                      negative_charged_scalar_plus_photon_lhc13(massj(j), couplingL, couplingR, iii, .True.)* &
                                      acceptance_type2(109171, massj(j), 0.0D0, Hplus, 0, 0, iii, -1) ! acceptance before mjj-cut

                        end do
                        allocate (final_state_rate_pos(6))
                        allocate (final_state_rate_neg(6))
                        mass_rec(j) = 0.0D0

                        do iii = 1, 6 ! Loop over partons for the final state
                            select case (iii)
                            case (1)
                                final_state_rate_pos(iii) = t%BR_Hpjud(jjj)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hplus, 0, 1, iii, +1)
                                final_state_rate_neg(iii) = t%BR_Hpjud(jjj)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hplus, 0, 1, iii, -1)
                            case (2)
                                final_state_rate_pos(iii) = t%BR_Hpjcs(jjj)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hplus, 0, 1, iii, +1)
                                final_state_rate_neg(iii) = t%BR_Hpjcs(jjj)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hplus, 0, 1, iii, -1)
                            case (3)
                                final_state_rate_pos(iii) = t%BR_Hpjus(jjj)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hplus, 0, 1, iii, +1)
                                final_state_rate_neg(iii) = t%BR_Hpjus(jjj)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hplus, 0, 1, iii, -1)
                            case (4)
                                final_state_rate_pos(iii) = t%BR_Hpjcd(jjj)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hplus, 0, 1, iii, +1)
                                final_state_rate_neg(iii) = t%BR_Hpjcd(jjj)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hplus, 0, 1, iii, -1)
                            case (5)
                                final_state_rate_pos(iii) = t%BR_Hpjub(jjj)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hplus, 0, 1, iii, +1)
                                final_state_rate_neg(iii) = t%BR_Hpjub(jjj)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hplus, 0, 1, iii, -1)
                            case (6)
                                final_state_rate_pos(iii) = t%BR_Hpjcb(jjj)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hplus, 0, 1, iii, +1)
                                final_state_rate_neg(iii) = t%BR_Hpjcb(jjj)* &
                                                            acceptance_type2(109171, massj(j), 0.0D0, Hplus, 0, 1, iii, -1)
                            end select
                            mass_rec(j) = mass_rec(j) + (final_state_rate_pos(iii)+final_state_rate_neg(iii))&
                                          *shifted_mass(S95_t2(c)%id, massj(j), particle_type(j), iii)
                        end do
                        mass_rec(j) = mass_rec(j)/(sum(final_state_rate_pos)+sum(final_state_rate_neg))
                        crosssection = posRate*sum(final_state_rate_pos)+negRate*sum(final_state_rate_neg)
                        deallocate (final_state_rate_neg)
                        deallocate (final_state_rate_pos)

                    else
                        stop "Error for table2 id 109171 and 109172: Unknown particle type!"
                    end if

                    crosssection = crosssection*efficiency_type2(S95_t2(c)%id, massj(j), 0.0D0, particle_type(j), 0) ! Detector efficiency

                    ! write(*,*) "S95_t2(c)%id, M, cross section = ",S95_t2(c)%id,  M, crosssection

                    if (crosssection .gt. vsmall) then
                        f = f + 1
                        fact = fact + crosssection
                        masstot = crosssection*mass_rec(j) + masstot
                        gammatot = gammatot + crosssection*t%particle(S95_t2(c)%particle_x2)%GammaTot(jjj)
                    end if
                end if
            end do
...
! (  Acceptance factor linearly increasing between 200 and 800 GeV, then constant)
        ...
        case (1903062481)
            fact = 1000.D0*t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j)*t%BR_hjmumu(j)
        case (1903062482)
            fact = 1000.D0*t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j)*t%BR_hjee(j)
        case (180406174)
            fact = test_appl(t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j)*t%BR_hkhjhi(j, i, i)*t%BR_hjbb(i)**2.0D0)
        case (200105178)
            fact = test_appl(1D3*t%lhc13%XS_vbf_ratio(j)*t%lhc13%XS_vbf_SM(j)*t%BR_hkhjhi(j, i, i)*t%BR_hjbb(i)**2.0D0)
        case default
        ...
        if (S95_t2(c)%particle_x1 .eq. not_a_particle) then
            ...
            case (01123, 170271, 170272, 1903062481, 1903062482) ! in percent
                axis_i = t%particle(S95_t2(c)%particle_x2)%GammaTot(j)/ &
                         t%particle(S95_t2(c)%particle_x2)%M(j)*100.0D0
            case (003892)
                axis_i = t%particle(S95_t2(c)%particle_x2)%GammaTot(j)/ &
                         t%particle(S95_t2(c)%particle_x2)%GammaTot_SM(j)
            case (160331, 160332, 170121)
                axis_i = t%particle(S95_t2(c)%particle_x2)%GammaTot(j)
            case (6065)
                axis_i = t%BR_Hpjtaunu(j)
            case (109171, 109172)
                axis_i = axis_i ! already calculated above
            case default
                stop 'Problem in subroutine calcfact_t2 (y1)'
            end select

            select case (S95_t2(c)%id)
            case (02301, 2016059, 4147, 20160791, 06386, 01123, 003892, 160331, 160332, 170121, 170271, 170272, &
                  1903062481, 1903062482, 109171, 109172)
    ...
    contains
        ...
        function test_appl(x)
            ...
            case (150, 160, 180, 190, 200, 210, 220, 230, 240, 3381, 3382, 13032, 06896, 14013, &
                  16002, 16032, 16029, 011811, 011812, 044781, 044782, 17002, 150600424, 16035, 110282, 180406174, 200105178)
            ...
            case (150011, 150012, 18012, 20160341, 20160342)
                if (massj(j) .lt. (massi(i) + 91.2D0)) then
                    test_appl = 0.0D0
                else
                    test_appl = x
                end if
...
!----Check for sufficiently SM-like BRs to gaga and bb of lighter Higgs
            ...
            case (170271)
!     write(*,*) "calcfact_t2 on 170271...", t%CP_value(j), x
                ! Limit only applies to CP-odd Higgs boson
                if ((t%CP_value(j) .ne. -1) .or. (x .le. 0.0D0)) then
                    test_appl = 0.0D0
                else
                    ! Limit assumes that top quark coupling dominates the gluon fusion production process (needs some testing!)
!       write(*,*) "Testing pp->A->tt limit (170271): ", abs(x - sqrt(t%lhc13%XS_gg_hj_ratio(j)))/x
                    if (abs(x - sqrt(t%lhc13%XS_gg_hj_ratio(j)))/x .le. 0.05D0) then
                        test_appl = x
                    else
                        test_appl = 0.0D0
                    end if
            ...
            case (170272)
!     write(*,*) "calcfact_t2 on 170272..."  , t%CP_value(j), x
                ! Limit only applies to CP-even Higgs boson
                if ((t%CP_value(j) .ne. 1) .or. (x .le. 0.0D0)) then
                    test_appl = 0.0D0
                else
                    ! Limit assumes that top quark coupling dominates the gluon fusion production process (needs some testing!)
!       write(*,*) "Testing pp->H->tt limit (1702712: ", abs(x - sqrt(t%lhc13%XS_gg_hj_ratio(j)))/x
                    if (abs(x - sqrt(t%lhc13%XS_gg_hj_ratio(j)))/x .le. 0.05D0) then
                        test_appl = x
                    else
                        test_appl = 0.0D0
    ...
    subroutine outputproc_t1(tlistn, jj, k, descrip)
        ...
        case (9219, 9463, 5472, 8957, 9596, 5828, 1970, 3493, 5972, 3155, 9868, 10068, 6092, 10217, 10239, &
              10796, 6220, 6309)
        ...
        case (4968)
            descrip = ' (p p)->t t-bar h'//j//'->t t-bar mu mu '//label
        case (1908064631)
            descrip = ' (p p) -> t t h'//j//' -> t t t t, where h'//j//' is CP-even '//label
        case (1908064632)
            descrip = ' (p p) -> t t h'//j//' -> t t t t, where h'//j//' is CP-odd '//label
        case (0611)
        ...
        case (1812, 2011138, 2011151, 2760, 2013090, 2014050, 14020, 11002, 11008, 160311, 79152)
            descrip = ' t->(H'//j//'+)b->tau nu b   '//label
        case (1269, 1270, 2011094, 13035, 18021)
        ...
        case (23801)
            descrip = ' (p p)->h'//j//'->V V (combination) '//label
        case (23802)
            descrip = ' (p p)->h'//j//'(VBF)->V V (combination) '//label
        case (20130132)
        ...
        case (170331)
            descrip = ' (p p)->h'//j//'/VBF -> W W -> lnuln + lnqq (employs NWA, assumes SM fraction pp->H vs. VBF)'//label
        case (170332)
            descrip = ' (p p)->h'//j//' -> W W -> lnuln + lnqq (employs NWA)'//label
        case (2016062)
        ...
        case (5105)
            descrip = ' (p p)->VBF/V h'//j//'-> (invisible) '//label
        case (59371)
            descrip = ' (p p)->VBF/V h'//j//'/ggh'//j//'-> (invisible) '//label
        case (59372)
            descrip = ' (p p)->VBF/ggh'//j//'-> (invisible) '//label
        case (13443)
        ...
        case (1508329, 2015080, 2749, 12191)
            descrip = ' (p p)->bbh'//j//'->b b '//label
        case (2016025, 180509299)
            descrip = ' (p p)->h'//j//'->b b '//label
        case (1810118221)
            descrip = ' (p p)->h'//j//' (CP-even) ->b b '//label
        case (1810118222)
            descrip = ' (p p)->h'//j//' (CP-odd) ->b b '//label
        case (2016044)
        ...
        case (31521, 81441)
            descrip = ' (g g)->h'//j//'->mu mu '//label
        case (31522, 81442)
            descrip = ' (b b)->h'//j//'->mu mu '//label
        case (180171, 61312)
            descrip = ' (g g)->h'//j//'->mu tau '//label
        case (1807065734)
            descrip = ' (p p)->h'//j//'->mu tau '//label
        case (180172, 61311)
            descrip = ' (g g)->h'//j//'->e tau '//label
        case (1807065733)
            descrip = ' (p p)->h'//j//'->e tau '//label
        case (102351, 180201122, 1807065731)
            descrip = ' (p p)->h'//j//'->e mu '//label
        case (1807065732)
            descrip = ' (g g)->h'//j//'->e mu '//label
        case default
...
! Added by OS 2012-03-12
    ...
    subroutine outputproc_t2(tlistn, ii, jj, k, descrip)
        ...
        case (200105178)
            descrip = ' (p p)->h'//j//'->h'//i//' h'//i//'->b b b b in VBF, where h'//i//' lies around 125 GeV  '//label
        case (011811)
            descrip = ' (p p)->h'//j//'->h'//i//' h'//i//'->b b tau tau, where h'//i//' lies around 125 GeV  '//label
        case (14013, 16002)
            descrip = ' (p p)->h'//j//'->h'//i//' h'//i//'->b b b b, where h'//i//' lies around 125 GeV  '//label
        case (14022)
            descrip = ' (p p)->h'//j//'->h'//i//' h'//i//'->tau tau tau tau, where h'//j// &
                      ' lies around 125 GeV and is SM-like '//label
        case (110282)
            descrip = ' (p p)->h'//j//'->h'//i//' h'//i//'->W W W W '//label
        case (011812, 150011)
        ...
        case (170271, 170272)
            descrip = ' (g g)->h'//j//'-> (t t-bar) (including widths effects) '//label
        case (003892)
        ...
        case (1903062481)
            descrip = ' (p p)->h'//j//'->mu mu (including widths effects) '//label
        case (1903062482)
            descrip = ' (p p)->h'//j//'->e e (including widths effects) '//label
        case (150600424, 16035)
            descrip = ' (p p)->h'//j//'->h'//i//'h'//i//'->mu mu mu mu  '//label
        case (109171)
            descrip = ' (q q)->(neutral or charged) scalar '//j//'->dijet resonance + photon (flavor inclusive) '//label
        case (109172)
            descrip = ' (q q)->h'//j//'->2b-jet resonance + photon '//label
        case default
    ...
    subroutine model_likeness(j, id, t, model_like, sigmaXbr)
        ...
        select case (id)
        case (711, 713, 721, 723, 731, 733, 741, 743, 5739, 10574, 6224, 6225, 6226, 6276, 6301, 6309, 18006, &
              04670, 046701, 046702, 336, 0038911, 0038912, 0038913, 0038914, 17030, 2025, 4873, 8567, 23801, 23802, 170331)
...
!             & , t%BR_Hjets_SM(j),0.0D0,1.0D0)
        ...
        case (5105)
            ...
            channel_rat(1, :) = (/t%lhc13%XS_hjZ_ratio(j), div(t%BR_hjinvisible(j), 1.0D0, 0.0D0, 1.0D0)/)
            channel_rat(2, :) = (/t%lhc13%XS_hjW_ratio(j), div(t%BR_hjinvisible(j), 1.0D0, 0.0D0, 1.0D0)/)
            channel_rat(3, :) = (/t%lhc13%XS_vbf_ratio(j), div(t%BR_hjinvisible(j), 1.0D0, 0.0D0, 1.0D0)/)

            channel_SM(1, :) = (/t%lhc13%XS_HZ_SM(j), 1.0D0/)
            channel_SM(2, :) = (/t%lhc13%XS_HW_SM(j), 1.0D0/)
            channel_SM(3, :) = (/t%lhc13%XS_vbf_SM(j), 1.0D0/)
        ...
        case (59371)
            ...
            channel_rat(2, :) = (/t%lhc13%XS_hjW_ratio(j), div(t%BR_hjinvisible(j), 1.0D0, 0.0D0, 1.0D0)/)
            channel_rat(3, :) = (/t%lhc13%XS_vbf_ratio(j), div(t%BR_hjinvisible(j), 1.0D0, 0.0D0, 1.0D0)/)
            channel_rat(4, :) = (/t%lhc13%XS_gg_hj_ratio(j), div(t%BR_hjinvisible(j), 1.0D0, 0.0D0, 1.0D0)/)

            channel_SM(1, :) = (/t%lhc13%XS_HZ_SM(j), 1.0D0/)
            channel_SM(2, :) = (/t%lhc13%XS_HW_SM(j), 1.0D0/)
            channel_SM(3, :) = (/t%lhc13%XS_vbf_SM(j), 1.0D0/)
            channel_SM(4, :) = (/t%lhc13%XS_gg_H_SM(j), 1.0D0/)

        case (59372)
            nc = 2; call initialise_channel_rat_SM
            channel_rat(1, :) = (/t%lhc13%XS_gg_hj_ratio(j), div(t%BR_hjinvisible(j), 1.0D0, 0.0D0, 1.0D0)/)
            channel_rat(2, :) = (/t%lhc13%XS_vbf_ratio(j), div(t%BR_hjinvisible(j), 1.0D0, 0.0D0, 1.0D0)/)

            channel_SM(1, :) = (/t%lhc13%XS_gg_H_SM(j), 1.0D0/)
...
!             & , t%BR_Hjets_SM(j) ,0.0D0,1.0D0)
            ...
            channel_rat(11, :) = (/t%tev%XS_hj_ratio(j), &
                                   div((t%BR_hjss(j) + t%BR_hjcc(j) + t%BR_hjbb(j) + t%BR_hjgg(j)), &
                                       t%BR_Hjets_SM(j), 0.0D0, 1.0D0)/)
...
!   ns = 1; nb = 2; call initialise_XS_rat_BR_rat
        ...
        case (170331)
            ...
            channel_rat(1, :) = (/t%lhc13%XS_hj_ratio(j), div(t%BR_hjWW(j), t%BR_HWW_SM(j), 0.0D0, 1.0D0)/)
            channel_rat(2, :) = (/t%lhc13%XS_vbf_ratio(j), div(t%BR_hjWW(j), t%BR_HWW_SM(j), 0.0D0, 1.0D0)/)

            channel_SM(1, :) = (/t%lhc13%XS_hj_ratio(j), t%BR_HWW_SM(j)/)
            channel_SM(2, :) = (/t%lhc13%XS_vbf_ratio(j), t%BR_HWW_SM(j)/)
...
!   BR_rat(1) = div( t%BR_hjgaga(j) , t%BR_Hgaga_SM(j) ,0.0D0,1.0D0)
        ...
        case (23801)
            ...
            channel_rat(1, :) = (/t%lhc13%XS_hj_ratio(j), div(t%BR_hjWW(j), t%BR_HWW_SM(j), 0.0D0, 1.0D0)/)
            channel_rat(2, :) = (/t%lhc13%XS_hj_ratio(j), div(t%BR_hjZZ(j), t%BR_HZZ_SM(j), 0.0D0, 1.0D0)/)

            channel_SM(1, :) = (/t%lhc13%XS_H_SM(j), t%BR_HWW_SM(j)/)
            channel_SM(2, :) = (/t%lhc13%XS_H_SM(j), t%BR_HZZ_SM(j)/)

        case (23802)
            nc = 2; call initialise_channel_rat_SM

            channel_rat(1, :) = (/t%lhc13%XS_vbf_ratio(j), div(t%BR_hjWW(j), t%BR_HWW_SM(j), 0.0D0, 1.0D0)/)
            channel_rat(2, :) = (/t%lhc13%XS_vbf_ratio(j), div(t%BR_hjZZ(j), t%BR_HZZ_SM(j), 0.0D0, 1.0D0)/)

            channel_SM(1, :) = (/t%lhc13%XS_vbf_SM(j), t%BR_HWW_SM(j)/)
            channel_SM(2, :) = (/t%lhc13%XS_vbf_SM(j), t%BR_HZZ_SM(j)/)
...
! This only checks the relative proportions of the four main production modes
        ...
        case (2025) ! This is a H->h(SM)h(SM) search!
            ns = 1; nb = 1; call initialise_XS_rat_BR_rat
            XS_rat(1) = t%lhc13%XS_hj_ratio(j)*t%lhc13%XS_H_SM(j) ! leave pb
            BR_rat(1) = 0.0D0
            correct_properties = .False.
            do jj = 1, np(Hneut)
                if (abs(t%particle(Hneut)%M(jj) - 125.0D0) .lt. (5.0D0 + t%particle(Hneut)%dMh(jj))) then
        ...
        case (0038911) ! H-WW search with width-assumptions
            ...
            if (t%particle(Hneut)%GammaTot(j) < 0.2D0*t%particle(Hneut)%GammaTot_SM(j)) then ! NWA requirement
                correct_properties = .True.
            else
                correct_properties = .False.
            end if
        ...
        case (0038913) ! H-WW search with width-assumptions
            ...
            XS_rat(1) = t%lhc8%XS_hj_ratio(j)*t%lhc8%XS_H_SM(j)
            BR_rat(1) = t%BR_hjWW(j)
            if (t%particle(Hneut)%GammaTot(j) > 0.8D0*t%particle(Hneut)%GammaTot_SM(j)) then ! NWA requirement
                correct_properties = .True.
            else
                correct_properties = .False.
            end if
        ...
        case (1810118221)
            ns = 1; nb = 1; call initialise_XS_rat_BR_rat
            XS_rat(1) = t%lhc13%XS_gg_hj_ratio(j)*t%lhc13%XS_gg_H_SM(j)
            BR_rat(1) = t%BR_hjbb(j)
            if (t%CP_value(j) .eq. 1) then ! analysis only applies if higgs is CP even
            else
                correct_properties = .False.
            end if
...
!-If channel_rat is not allocated, use old method:
    ...
    contains
        ...
        subroutine initialise_XS_rat_BR_rat
            if (allocated(XS_rat)) print *, "AAAAAAAHHHHHHH"
            allocate (XS_rat(ns))
            allocate (BR_rat(nb))
            XS_rat = unset
            BR_rat = unset
    ...
    !***********************************************************
    subroutine fill_blank_ft1_dat(ft1, ft1_sep, vmasslower, vmasshigher, vmass_xmin, vmass_sep, valueoutsidetable)
        ! don't forget to deallocate f_t1%dat
        implicit none
        integer :: ilower, ihigher
        double precision, intent(in) :: ft1_sep, vmasslower, vmasshigher, vmass_xmin, vmass_sep, valueoutsidetable
        type(table1) :: ft1
    ...
    subroutine f_from_t1(t1, vmasslower, vmasshigher, sepmultfactor, datcomp, &
        ...
        call fill_blank_ft1_dat(f_t1, f_t1%sep, vmasslower, vmasshigher, vmass_xmin, vmass_sep, valueoutsidetable)
    ...
    subroutine f_from_t2(t2, m1_at_ref_point_1, m2_at_ref_point_1, m1_at_ref_point_2, m2_at_ref_point_2, &
        ...
        if (abs(m1_at_ref_point_1 - m1_at_ref_point_2) .lt. small) then
            const_m1 = .True.
            line_grad = 0 ! not needed
            line_const = 0 ! not needed
        else
            const_m1 = .False.
    ...
    subroutine f_from_slices_t2(slices_t2, m1_at_ref_point_1, m2_at_ref_point_1, m1_at_ref_point_2, m2_at_ref_point_2, z, &
        ...
        if (.not. ((z .ge. slices_t2(1)%z - small) .and. (z .le. slices_t2(2)%z + small))) then !#1! written in convoluted way to get the NaNs
            ...
            elseif (abs(z_above - z) .lt. small) then !z is the same as z_above    !#2
                ...
                if (const_m1) then
                    line_grad = 0.D0 ! not needed
                    line_const = 0.D0 ! not needed
                else
                    line_grad = (m2_at_ref_point_1 - m2_at_ref_point_2)/(m1_at_ref_point_1 - m1_at_ref_point_2)
                    line_const = (m1_at_ref_point_1*m2_at_ref_point_2 - m1_at_ref_point_2*m2_at_ref_point_1) &
                                 /(m1_at_ref_point_1 - m1_at_ref_point_2)
                ...
                call fill_blank_ft1_dat(f_t1, f_t1%sep, &
                                        vmasslower, vmasshigher, &
                                        vmass_xmin, vmass_sep, &
                                        valueoutsidetable)
    ...
    subroutine f_from_t3(t3, m1_at_ref_point_1, m2_at_ref_point_1, m1_at_ref_point_2, m2_at_ref_point_2, z, &
        ...
        integer :: a
        logical :: const_m1, const_m2
        double precision :: vmass_xmin, vmass_sep
