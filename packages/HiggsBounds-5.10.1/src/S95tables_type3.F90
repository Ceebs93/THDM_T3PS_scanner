...
contains
    ...
    !************************************************************
    subroutine initializetables3(clsb)
        !***********************************************************
        implicit none
        ...
        xbeg = lbound(clsb, dim=1)
        xend = ubound(clsb, dim=1)
        ...
        clsb(x)%id = 1800
        clsb(x)%type_of_assoc_table = 2
        clsb(x)%id_of_assoc_table = 180
        clsb(x)%xmax2 = 180.0D0
        clsb(x)%xmin2 = 1.0D0
        clsb(x)%xmax1 = 180.0D0
        clsb(x)%xmin1 = 1.0D0
        clsb(x)%sep1 = 1.0D0
        clsb(x)%sep2 = 1.0D0
        ...
        clsb(x)%id = 2000
        clsb(x)%type_of_assoc_table = 2
        clsb(x)%id_of_assoc_table = 200
        clsb(x)%xmax2 = 180.0D0
        clsb(x)%xmin2 = 1.0D0 ! except there's no numbers for this since Mh2 must be > Mh1
        clsb(x)%xmax1 = 90.0D0
        clsb(x)%xmin1 = 1.0D0
        clsb(x)%sep1 = 1.0D0
        clsb(x)%sep2 = 1.0D0
        ...
        clsb(x)%id = 1420
        clsb(x)%type_of_assoc_table = 1
        clsb(x)%id_of_assoc_table = 142
        clsb(x)%xmax1 = 180.0D0
        clsb(x)%xmin1 = 0.1D0
        clsb(x)%xmax2 = 20.0D0
        clsb(x)%xmin2 = 20.0D0
        clsb(x)%sep2 = 1.0D0 !doesn't matter what this value is as long as it is not zero
        clsb(x)%sep1 = 0.1D0
        ...
        do x = xbeg, xend
            ...
            clsb(x)%nx2 = nint((clsb(x)%xmax2 - clsb(x)%xmin2)/clsb(x)%sep2) + 1
            clsb(x)%nx1 = nint((clsb(x)%xmax1 - clsb(x)%xmin1)/clsb(x)%sep1) + 1
            clsb(x)%nz = nint((clsb(x)%zmax - clsb(x)%zmin)/clsb(x)%zsep) + 1

            if (abs(clsb(x)%sep1) .lt. small) stop 'problem in subroutine initializetables3 b1'
            if (abs(clsb(x)%sep2) .lt. small) stop 'problem in subroutine initializetables3 b2'
            if (abs(clsb(x)%zsep) .lt. small) stop 'problem in subroutine initializetables3 b3'
            if (clsb(x)%xmax1 .lt. clsb(x)%xmin1) stop 'problem in subroutine initializetables3 b4'
            if (clsb(x)%xmax2 .lt. clsb(x)%xmin2) stop 'problem in subroutine initializetables3 b5'
            if (clsb(x)%zmax .lt. clsb(x)%zmin) stop 'problem in subroutine initializetables3 b6'

            !problem: can't set clsb(x)%needs_M2_gt_2M1 here because it needs
            !S95_t2
            !if(clsb(x)%type_of_assoc_table.eq.2)then
            !  table2num=t2elementnumberfromid(S95_t2,clsb(x)%id_of_assoc_table)
            !  clsb(x)%needs_M2_gt_2M1 =  S95_t2(table2num)%needs_M2_gt_2M1
            !endif
    ...
    subroutine fill_t1dat_from_t3(t1, t3, zi, ftype)
        ...
        !***********************************************************
        use usefulbits, only: vsmall, vvsmall
        !--------------------------------------input
        type(table3), intent(in) :: t3
        ...
        if (abs(t1%sep - t3%sep1) .gt. vsmall) stop 'problem in fill_t1dat_from_t3 (2d)'
    ...
    subroutine readclsbfiles_binary
        ...
        do z = lbound(clsb_t3, dim=1), ubound(clsb_t3, dim=1)
            ...
            foldername = 'lep-chisq-master/csboutput_trans_binary/csboutput_outfile_fullclsb_ee_'//trim(adjustl(filename(z)))
            open (file_id_3, file=trim(adjustl(pathname))// &
    ...
    subroutine readclsbfiles(clsb_t3_id)
        ...
        do y = 1, ymax
            do x = 1, deltaMhinfilenamebits
                ...
                Mhinfilename_initial = Mhinfilename_xmin + &
                                       dble(y - 1)*deltaMhinfilename &
                                       + dble(x - 1)*deltaMhinfilename/dble(deltaMhinfilenamebits)
                ...
                case (2)
                    do j = 1, jmax
                        imax(j) = int((Mhinfilename_initial + clsb_t3(z)%sep2*(j - 1)) &
                                      *(clsb_t3(z)%xmax1/clsb_t3(z)%xmax2)/clsb_t3(z)%sep2)
                    enddo
                case default
                    stop 'error in subroutine readclsbfiles'
                end select
                ...
                select case (Mhbeg)
                case (0:9)
                    Mhbegstring = '00'//trim(adjustl(Mhbegstring))
                case (10:99)
                    Mhbegstring = '0'//trim(adjustl(Mhbegstring))
                case default
                end select

                select case (Mhend)
                case (0:9)
                    Mhendstring = '00'//trim(adjustl(Mhendstring))
                case (10:99)
                    Mhendstring = '0'//trim(adjustl(Mhendstring))
                case default
                end select
                ...
                do j = 1, jmax
                    ...
                    do i = 1, imax(j)
                        ...
                        do n = 1, clsb_t3(z)%nz
                            ...
                            select case (clsb_t3(z)%type_of_assoc_table)
                            case (1)
                                if (abs(Mh1_in - (Mhinfilename_initial + dble(i - 1)*clsb_t3(z)%sep1)) .gt. 0.0001D0) then
                                    stop 'error: Mh1_in'
                                endif
                                if (abs(Mh2_in - (clsb_t3(z)%xmin2 + dble(j - 1)*clsb_t3(z)%sep2)) .gt. 0.0001D0) then
                                    stop 'error: Mh2_in'
                                endif
                            case (2)
                                if (abs(Mh1_in - (clsb_t3(z)%xmin1 + dble(i - 1)*clsb_t3(z)%sep1)) .gt. vsmall) then
                                    stop 'error: Mh1_in'
                                endif
                                if (abs(Mh2_in - (Mhinfilename_initial + dble(j - 1)*clsb_t3(z)%sep2)) .gt. 0.0001D0) then
                                    stop 'error: Mh2_in'
                                endif
                            case default
                                stop 'error in subroutine readclsbfiles'
                            ...
                            if (abs(log10sf_in - (clsb_t3(z)%zmin + clsb_t3(z)%zsep*dble(n - 1))) .gt. 0.0001D0) then
                                stop 'error: log10sf_in'
                            endif
#endif
