...
contains
    ...
    subroutine setup_output
        ...
        case ('datfile')
            ...
            rep = numres       ! TS 13/09/2019: changed from 1 to numres=3 to get info from next-sensitive channels
        case ('subrout')
            !create Key to process numbers in file Key.dat
#ifndef WEBVERSION
            if (wantkey) then
...
!--NEW in HS-4:
    ...
    subroutine do_output
        ...
        case ('datfile')
            ...
            case ('effC', 'part', 'hadr')
                ...
                write (file_id_common, *) '# generated with HiggsBounds version '//trim(adjustl(version))//' on '//createdateandtime
                ...
                if (n_additional > 0) then
                    write (file_id_common, *) '#   additional : optional additional data stored in <prefix>additional.dat'
                endif
                write (file_id_common, *) '#'
    ...
    subroutine createKey(file_beginning)
        ...
        use usefulbits, only: np, ntot, pr, whichanalyses !input
...
134     format('**************************************************************')
        ...
        write (file_id_common, *) 'This key has been generated with HiggsBounds version '//trim(adjustl(version))//' '
        ...
        do n = 1, ntot
            ...
            if (pr(n)%ttype .eq. 1) then
                ...
                write (file_id_common, *) trim(descrip), " \cite{",trim(adjustl(S95_t1(pr(n)%tlist)%citekey)),"}"
            elseif (pr(n)%ttype .eq. 2) then
                if ((i .eq. j) .and. (S95_t2(pr(n)%tlist)%needs_M2_gt_2M1)) then
                    ...
                    call outputproc(pr(n), file_id_common, descrip, 1)
                    write (file_id_common, *) trim(descrip), " \cite{",trim(adjustl(S95_t2(pr(n)%tlist)%citekey)),"}"
                endif
            elseif (pr(n)%ttype .eq. 9) then
                call outputproc_llh(pr(n), file_id_common, descrip)
                write (file_id_common, *) trim(descrip), " \cite{",trim(adjustl(llhdata(pr(n)%tlist)%D2llhdata(1)%citekey)),"}"
            else
            ...
            select case (pr(n)%ttype)
            case (1)
                if ((i .eq. np(S95_t1(pr(n)%tlist)%particle_x)) &
                    .and. (j .ne. np(S95_t1(pr(n)%tlist)%particle_x))) then
                    write (file_id_common, 133)
                endif
            case (2)
                if ((i .eq. np(S95_t2(pr(n)%tlist)%particle_x1)) &
                    .and. (j .ne. np(S95_t2(pr(n)%tlist)%particle_x2))) then
                    write (file_id_common, 133)
                endif
            end select
        enddo
