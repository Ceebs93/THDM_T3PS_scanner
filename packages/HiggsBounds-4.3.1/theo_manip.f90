...
! DEBUG
! Warning: this is just for debugging, need to remove later.
!  write(*,*) 'np(Hneut):', np(Hneut)


  do i=1,np(Hneut)
     
!   write(*,*) 'Before theo calculation'
   Mhi=theo(jj)%particle(Hneut)%M(3)    
...
!     write(*,*) 'Look here!!!'
!     write(*,*) 'Mhi:', Mhi
!     write(*,*) 'g2(1)%hjbb_s(3)', g2(jj)%hjbb_s(3)
!     write(*,*) 'g2(1)%hjbb_p(3)', g2(jj)%hjbb_p(3)
!     write(*,*) 'g2(jj)%hjtautau_p(i)', g2(jj)%hjtautau_p(i)
!     write(*,*) 'invbsq(mbmb,mHi)', invbsq(mbmb,Mhi)
!     write(*,*) 'BRSM_Hbb(Mhi)', BRSM_Hbb(Mhi)
!     write(*,*) 'GammaRat', GammaRat
!     write(*,*) 'theo(jj)%particle(Hneut)%GammaTot(i)', theo(jj)%particle(Hneut)%GammaTot(i)
!     write(*,*) 'BRSM_GammaTot(Mhi)',                  BRSM_GammaTot(Mhi)  
!  endif 
 ...
 !*****************************************************************
  ...
  if(np(Hneut)>0)then
   ...
   ! DEBUG
   !write(*,*) 'ndat', ndat
   !write(*,*) 'theo(1)%particle(Hneut)%M(3)', theo(1)%particle(Hneut)%M(3) 
   !write(*,*) 'theo(1)%BR_hjss    ',     theo(1)%BR_hjss
   !write(*,*) 'theo(1)%BR_hjcc    ',     theo(1)%BR_hjcc
   !write(*,*) 'theo(1)%BR_hjbb    ',     theo(1)%BR_hjbb
   !write(*,*) 'theo(1)%BR_hjmumu  ',   theo(1)%BR_hjmumu
   !write(*,*) 'theo(1)%BR_hjtautau', theo(1)%BR_hjtautau
   !write(*,*) 'theo(1)%BR_hjWW    ',     theo(1)%BR_hjWW
   !write(*,*) 'theo(1)%BR_hjZZ    ',     theo(1)%BR_hjZZ
   !write(*,*) 'theo(1)%BR_hjZga   ',    theo(1)%BR_hjZga
   !write(*,*) 'theo(1)%BR_hjhihi  ',   theo(1)%BR_hjhihi
...
!      if(debug)
! Warning: I manually removed this warning
!          This caused the MCMC to get stuck previously!
