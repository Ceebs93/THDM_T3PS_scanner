...
!--------------------------------------------------------------------
  ...
  use usefulbits, only: np,Hneut,Hplus,file_id_common3,file_id_common2
!   implicit none
...
!  type(mutable),allocatable :: tables(:)
  !-----------------------------------internal
  logical :: newtables
  integer :: x, n, xbeg, xend, i, m, int1, int2
...
!&	 'Expt_tables/'//trim(adjustl(Exptdir))//'/*.txt` > HS_analyses.txt')
   call system('ls -1 -p '//trim(adjustl(pathname_HS))// &
&	 'Expt_tables/'//trim(adjustl(Exptdir))//'/*.txt 2>/dev/null | xargs -L 1 basename > HS_analyses.txt 2>/dev/null')
...
!  print *, "Basename 300"
   call system('ls -1 -p '//trim(adjustl(pathname_HS))// &
&	 'Expt_tables/'//trim(adjustl(Exptdir))//'/*.corr 2>/dev/null | xargs -L 1 basename > HS_correlations.txt 2>/dev/null')
!  print *, "Basename 301"
!  call system('rm -rf HS_ncorrelations.txt')
