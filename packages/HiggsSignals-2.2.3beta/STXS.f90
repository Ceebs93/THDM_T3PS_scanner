...
!print *, "Basename 000"
! call system('basename `ls -1 -p '//trim(adjustl(pathname_HS))// &
!&	 'Expt_tables/'//trim(adjustl(dataset))//'/*.stxs 2>/dev/null` > STXS_analyses.txt 2>/dev/null')
 call system('ls -1 -p '//trim(adjustl(pathname_HS))// &
&	 'Expt_tables/'//trim(adjustl(dataset))//'/*.stxs 2>/dev/null | xargs -L 1 basename > STXS_analyses.txt 2>/dev/null')
!print *, "Basename 001"
...
! print *, "Basename 100"
   call system('ls -1 -p '//trim(adjustl(pathname_HS))// &
&	 'Expt_tables/'//trim(adjustl(dataset))//'/*.stxscorr 2>/dev/null | xargs -L 1 basename > STXS_correlations.txt 2>/dev/null')
! print *, "Basename 101"
