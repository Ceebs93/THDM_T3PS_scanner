...
int main(int argc, char* argv[])                                                                   
{
	...
	else if ( argc == 8 )   // - parameters as input
	{
		...
		m12_2_in   = (double)atof(argv[9]);
		tb_in      = (double)atof(argv[10]);
	}
	
	else
	{
	  printf("ParameterScan_T3PS usage:\n");
	  printf("ParameterScan_T3PS <yt_in> <l1> <l2> <l3> <l4> <l5> <l6> <l7> <tanb>");
	}
	...
	const HBHSResult *hbhsres_ptr = nullptr;
	  ...
	  if (yt_in == 2 && (l6_in == 0 or l7_in == 0)){
		printf("WARNING: Requested Yukawa type respects Z2-symmetry but lambda6 or lambda7 is not zero");
	  }		
