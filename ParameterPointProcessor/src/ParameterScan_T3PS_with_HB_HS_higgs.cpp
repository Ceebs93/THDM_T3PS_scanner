...
{
	int       yt_in;   // - Type
	double    l1_in;   // - lambda1
	double    l2_in;   // - lambda2
	double    l3_in;   // - lambda3
	double    l4_in;   // - lambda4
	double    l5_in;   // - lambda5
	double    l6_in;   // - lambda6
	double    l7_in;   // - lambda7
	double    mHc_in;  // - charged higgs mass
	...
	else if ( argc == 8 )   // - parameters as input
	{
		...
		l2_in     = (double)atof(argv[3]);
		l3_in     = (double)atof(argv[4]);
	  	l4_in     = (double)atof(argv[5]);
	   	l5_in     = (double)atof(argv[6]);
	   	l6_in     = (double)atof(argv[7]);
	   	l7_in     = (double)atof(argv[8]);
		mHc_in    = (double)atof(argv[9]);
	}
	
	else
	{
	  printf("ParameterScan_T3PS usage:\n");
	  printf("ParameterScan_T3PS <yt_in> <l1> <l2> <l3> <l4> <l5> <l6> <l7> <mHc>");
	}
	...
	const HBHSResult *hbhsres_ptr = nullptr;
	  ...
	  bool pset = model.set_param_higgs(l1_in,l2_in,l3_in,l4_in,l5_in,l6_in,l7_in,mHc_in);
	...
	printf("| cba   |         |        |         | %+8.3e |\n", cba_hybrid);
	printf("| sba   |         |        | %+8.3e  |        |\n", sba_phys);
	printf("| tb    |         | %+8.3e | %+8.3e  | %+8.3e |\n", tb_gen, tb_phys, tb_hybrid);
