...
//	# endif
	...
	std::cerr << "Before HB_set_unput_effC " << std::endl;
	// -- Share couplings of 2HDM model with HiggsBounds/HiggsSignals
	HB_set_input_effC(model);
	std::cerr << "After HB_set_unput_effC " << std::endl;
...
//	<< br_A_sum  <<                     // tmp
	...
	// write_LesHouches(const char* file, bool fulldecay, bool couplings, bool qcd_on, bool HBHS)
	const char lha_output[] = "temp.lha";
	model.write_LesHouches(lha_output, true, true, true, false);
