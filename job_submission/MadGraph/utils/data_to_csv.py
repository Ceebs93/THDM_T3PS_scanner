...
def add_xsect(Filename, Chkdfile, proc):
    ...
    if len(MG_df['Sbma']) == len(Chkd_df.sinba):
        if 'X_sections' in MG_df.columns:
            ...
            Chkd_df = Chkd_df.assign(str(process)=x_column)
        else:
            print("Error! Could not find a column 'X_sections' in provided 
                   file")
    else:
...
def make_useful(Filename, inc_Hc, THDMC_output=False):
    ...
    # Can alter the list of column names for runs where processes invovle diff-
    # erent variables.
    df = pd.read_csv(Filename, sep=",", names= ['Z7', 'mH', 'mHc', 'mA', 'cba', 
                     'tb', 'sinba', 'Z4', 'Z5', 'm12_2', 'l1', 'l2', 'l3', 
                     'l4', 'l5', 'l6', 'l7', 'g_HpHmh', 'Gamma_h', 'Gamma_H', 
                     'Gamma_Hc', 'Gamma_A', 'br_h_bb', 'br_h_tautau', 
                     'br_h_gg', 'br_h_WW', 'br_h_ZZ', 'br_h_gaga', 'br_A_tt', 
                     'br_A_bb', 'br_A_gg', 'br_A_mumu', 'br_A_tautau',
                     'br_A_Zga', 'br_A_Zh', 'br_A_ZH', 'br_A_gaga', 'br_H_tt', 
                     'br_H_bb', 'br_H_gg', 'br_H_mumu', 'br_H_tautau',
                     'br_H_Zga', 'br_H_Zh', 'br_H_WW', 'br_H_ZZ', 'br_H_ZA', 
                     'br_H_hh', 'br_H_AA', 'br_H_gaga', 'br_Hp_tb',
                     'br_Hp_taunu', 'br_Hp_Wh', 'br_Hp_WH', 'br_Hp_WA', 'sta',
                     'uni', 'per_4pi', 'per_8pi', 'S', 'T', 'U', 'V', 'W', 'X',
                     'delta_rho', 'delta_amu', 'tot_hbobs', 'sens_ch', 
                     'chi2_HS', 'chi2_ST_hepfit', 'chi2_ST_gfitter', 
    ...
    # and negative values of sinba. These are then recombined. Values used can be 
    # found in REFERENCEFORLIMITSHERE
    ...
    # When excluding the charged higgs we require masses to be above 580GeV REFERENCEFORTHISHERE
    if inc_Hc == False:
    ...
    theory_mask = (temp_df["per_8pi"] == per_4pi) & (temp_df["sta"] == sta) & \
                  (temp_df["uni"] == uni)
    ...
    print("Number of data points in the reduced DataFrame: {}".format(len(
           reduced)) )
...
def combiner(df_list):
    """ Takes a list of pandas dataframes, combines them and removes any dupli-
        cate rows"""
...
def user_interface():
    ...
    if option == 1:
        ...
        print("1) A .dat file    2) A .csv post MG5)")

        file_type = int(input("Please enter 1 or 2 only: "))
        ...
        nw_name = input("You do not need to add .csv, this will be done autom-
        atically ")
